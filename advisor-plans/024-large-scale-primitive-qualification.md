...
- **Implementation status**: versioned profiles, deterministic streaming workload execution, measured run artifacts, preflight guards, pinned-signer validation, receipt-to-workload seed binding, and a local typed all-primitive smoke driver are implemented; protected provider/Kubernetes/scale receipts remain open
