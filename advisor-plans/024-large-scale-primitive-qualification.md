...
- **Implementation status**: schema-2 versioned profiles now bind provider/topology identity, throughput, RSS, local-disk, file-descriptor, and object-store-call envelopes; schema-2 matrix manifests now require canonical row order and the CLI can build them from a real evidence tree; matrix verification fails closed on missing or over-limit measurements, and measured run artifacts enforce their throughput envelope in addition to per-primitive progress and p99/duration checks. Run-artifact schema 4 now binds a bounded primitive/lifecycle-case bitset and protected resource measurements, and named provider/topology profiles reject measured runs that omit any scheduled case or resource observation or leave an acknowledged operation independently unverified; typed adapters can opt into serial or concurrent case-coverage enforcement. Deterministic streaming workload execution (including a bounded concurrent runner for independent/idempotent operations) now exposes a seed-bound lifecycle schedule covering happy, retry, duplicate, expiry, cancellation, owner-loss, and recovery cases for every primitive in the default workload; preflight guards, public black-box receipt/matrix mismatch coverage, public protected-profile/run-artifact matrix coverage, pinned-signer validation, receipt-to-workload seed binding, receipt/run-artifact metric binding for the primitives row (including RSS, disk, file descriptors, and object-store calls), a typed all-primitive smoke driver through the public `CellNode` host with readiness-gated evidence admission, an honest `run_qualification_observed` smoke path that leaves unobserved lifecycle bits unset, a canonical `QualificationRunner::emit_protected_run` binder that rejects mismatched run/workload artifacts, and the executable `qualification_receipt bind-protected` path for canonical non-secret evidence and an external signing key are implemented; named provider profiles additionally require a canonical provider-semantics artifact bound to the provider, profile digest, workload seed, and successful conditional/range/multipart checks; release packaging of the verified protected evidence bundle and exact tagged-source binding for the protected `scale-v1` profile are also implemented; protected provider/Kubernetes/scale receipts remain open. Provider-specific S3, GCS, and Azure correctness/fault profiles now bind the advertised provider and Kubernetes topology independently; protected runs and retained receipts for those profiles remain open.

The release workflow now has an explicit manual-run or repository-variable
handoff for retained protected evidence. The canonical manual workflow uses a
protected self-hosted runner and an operator-installed provider/Kubernetes
harness, checks the exact source and image identity, verifies every required
profile in a fresh process, and uploads the required artifact layout. It fails
...
and after settlements; each restores every Cell through `CellNode` and verifies
the acknowledged or absent outcomes from a successor and an independent
observer. These are repeatable host/lifecycle regression gates, not provider,
Kubernetes, or scale evidence; those protected rows still require a retained
signed run artifact.
