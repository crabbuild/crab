...
- **Implementation status**: provider-neutral host owns serving and offline-maintenance runtime construction, exposes canonical `start`/`status` lifecycle calls, requires its coordination supervisor before readiness, and serializes deadline-aware shutdown/drain callers; full operator-facility ownership and multi-process public-host qualification remain open
