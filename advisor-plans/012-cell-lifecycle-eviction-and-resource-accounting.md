...
The ignored `rustfs_mixed_primitive_inventory_churn_preserves_exact_roots`
qualification reuses that same helper with an isolated RustFS prefix. It passed
with the Queue and Workflow rows retained, both Cells drained, the Queue root
reacquired byte-for-byte, and a third Cell admitted after release. This is
provider-backed primitive accounting evidence; a larger provider-scale mixed
workload and protected release receipt remain open.
