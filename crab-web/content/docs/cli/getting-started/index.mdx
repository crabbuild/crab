...
# 2. Configure a repository with your bucket
cd my-project
crab configure s3://my-bucket/my-repo

# 3. Ship your first commit
crab ship . -m "initial commit"
```

`crab configure` guides you through cloud selection, checks credential
discovery, wires the repository to the remote, and scans for large files.
`crab ship` handles staging, committing, and pushing in one command. You can
still run `crab init` and `crab setup` separately when you want finer control.
