...
|------|-------------|------------------------|
| G0 | Checkpoint/artifact semantic loss is rejected or preserved; focused migration, YAML, and dispatch tests exist. `cargo test -p crab --lib cmd::migrate::tests --locked` (16), `cargo test -p crab-workflow dvc_inventory --locked` (18), and `cargo test -p crab-workflow artifact --locked` (16) pass locally. Recursive inventory now blocks nested `dvc.yaml` files because the converter accepts one pipeline file, preserves nested `.dvc` import provenance/DB metadata, and `from-dvc --stdout` keeps warnings on stderr. Unfinished history-rewrite commands now fail explicitly instead of returning success. | Run the complete migration fixture matrix and CI evidence audit |
...
| G5 | Bounded streaming/hash-index paths and honest unsupported-provider preflight exist; artifact declarations now have a primary-remote manifest/payload/stage/history boundary with strict CAS promotion, recoverable pending promotion markers, verified file/directory downloads, and GC reachability protection for refs, manifests, payloads, history, and in-flight markers. Reachability now fails closed when a referenced payload object is missing. Untrusted stage cache manifests are validated for repository-safe paths, canonical hashes, tree metadata, duplicate paths, and byte sizes before materialization, and non-miss remote cache integrity/materialization errors now propagate instead of silently rerunning the stage. Artifact/cache-validator/materialization suites pass locally, and the release RustFS command smoke passes locally. | Live SSH/SFTP/WebDAV/HDFS/Drive/OSS qualification remains outstanding |
| G6 | `crab data` contracts and local/file/HTTP/object-store transactional paths exist; `import-url` and `update` stream object-store bodies through temp files with size/validator verification, the bundled read-only SQLite connector materializes deterministic JSONL transactionally, and local Git revision imports materialize files, directories, and repository roots with executable-bit preservation. The command now resolves the current Git worktree root from nested directories, and `data status` reports explicit workspace/descriptor/Git/lock/cache/source/remote dimensions without implicit network access. Data unit tests (16), including object-store streaming, Git tree materialization, root discovery, and the status matrix, pass locally. | Level-3 repository/remote and broader database connector E2E; unsupported connectors must remain documented as unsupported |
