...
|------|-------------|------------------------|
| G0 | Checkpoint/artifact semantic loss is rejected or preserved; focused migration, YAML, and dispatch tests exist. `cargo test -p crab --lib cmd::migrate::tests --locked` (16), `cargo test -p crab-workflow dvc_inventory --locked` (16), and `cargo test -p crab-workflow artifact --locked` (16) pass locally. Recursive inventory now blocks nested `dvc.yaml` files because the converter accepts one pipeline file, and `from-dvc --stdout` keeps warnings on stderr. Unfinished history-rewrite commands now fail explicitly instead of returning success. | Run the complete migration fixture matrix and CI evidence audit |
