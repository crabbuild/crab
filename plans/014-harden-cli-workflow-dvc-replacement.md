...
| G3 | Authenticated checkpoint protocol, lineage, apply/reset/resume, local transport, checkpoint-aware local live-set validation, and conservative remote workflow root registration exist. `cargo test -p crab --test workflow_exp --locked` (36), checkpoint command tests (5), workflow checkpoint tests (8), and workflow GC tests (7) pass locally, including staged-reset publication failure rollback and checkpoint push/pull round-trip coverage. | Three-checkpoint crash/resume, push/pull, and GC E2E evidence against the release remote; directory-swap crash recovery is still unproven |
...
| G5 | Bounded streaming/hash-index paths and honest unsupported-provider preflight exist; local artifact registry is present; experiment publication records conservative workflow roots in the shared ref-registry. Artifact (16), validator-scope (1), and ref-registry focused suites pass; provider adapters and artifacts are not yet one shared remote boundary. | Shared adapter integration, remote artifact refs/GC, and live SSH/SFTP/WebDAV/HDFS/Drive/OSS qualification |
| G6 | `crab data` contracts and local/file/HTTP transactional paths exist. Data unit tests (10), TypeScript typecheck, and docs link checks pass locally; DB remains an explicit unsupported connector. | Level-3 repository/remote and database connector E2E; unsupported connectors must remain documented as unsupported |
