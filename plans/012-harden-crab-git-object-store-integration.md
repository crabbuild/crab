# Plan 012: Harden Crab's Git object-store integration
...
- **Completed**: 2026-08-14; live RustFS report
  `/Volumes/Workspace/CrabCLI/git-integration-20260814-2142/artifacts/report.json`
- **Re-audited and mitigated**: 2026-08-15; 82-check live RustFS report
  `/Volumes/Workspace/CrabCLI/plan012-mitigation-20260815-0904/artifacts/report.json`

### Post-completion audit mitigations

The re-audit closed six gaps found after the original completion run:

1. Shallow-boundary replacement/removal now commits only after fetched ref-tip
   validation, uses atomic file replacement, and preserves the old boundary
   when validation or the replacement write fails.
2. Remote-helper push and fetch errors collapse control characters so every
   status remains one protocol line.
3. A published pack index is parsed and checksum-verified, and its pack
   checksum must match the downloaded pack before installation succeeds.
4. Unsupported date/ref-exclusion shallow selectors terminate the helper
   session after an explicit error, because Git otherwise treats the option
   rejection as advisory and silently performs a full fetch.
5. Reachable-SHA admission now loads the published commit-graph summary in the
   production fetch path when the policy is enabled.
6. Native `crab push --follow-tags` is wired to strict tag publication and
   follows Git's contract: eligible annotated tags may target any commit
   reachable from the refs being pushed, including already-remote ancestors.

The expanded RustFS matrix corrupts a non-empty remote `.idx`, exercises the
default-denied and configured-allowed reachable-SHA paths, proves
`--shallow-since` fails before checkout, verifies both Git and native Crab
follow-tags, and retains the 64 MiB clone/hydrate byte-identity proof.
...
|---------|---------|---------------------|
| Workspace preflight | `test -d /Volumes/Workspace/crabbuild-target && mkdir -p /Volumes/Workspace/crabbuild-target/crab-git-integration-production-hardening && test -w /Volumes/Workspace/crabbuild-target/crab-git-integration-production-hardening` | exit 0 |
| Format check | `cargo fmt --all --check` | exit 0, no diff |
| Focused helper tests | `CARGO_TARGET_DIR=/Volumes/Workspace/crabbuild-target/crab-git-integration-production-hardening cargo test -p crab --locked --test remote_helper_transcript --test e2e_add_commit_push --test e2e_fetch_fsck` | all tests pass, zero ignored in `remote_helper_transcript` |
| Feature contract | `CARGO_TARGET_DIR=/Volumes/Workspace/crabbuild-target/crab-git-integration-production-hardening cargo test -p crab --locked --features gix-transport --test v2_fetch_transport` | all tests pass, zero ignored |
| Crab Git library tests | `CARGO_TARGET_DIR=/Volumes/Workspace/crabbuild-target/crab-git-integration-production-hardening cargo test -p crab --locked --lib git::` | all selected tests pass |
| Tag owner tests | `CARGO_TARGET_DIR=/Volumes/Workspace/crabbuild-target/crab-git-integration-production-hardening cargo test -p crab-git --locked tag` | all selected tests pass |
| Clippy | `CARGO_TARGET_DIR=/Volumes/Workspace/crabbuild-target/crab-git-integration-production-hardening cargo clippy -p crab -p crab-git --all-targets --locked -- -D warnings` | exit 0, no warnings in touched code |
| Release build | `CARGO_TARGET_DIR=/Volumes/Workspace/crabbuild-target/crab-git-integration-production-hardening cargo build -p crab --release --locked` | release binary exists under the selected external target |
| Live qualification | `python3 crab/scripts/e2e/run_add_commit_push_rustfs_smoke.py --root /Volumes/Workspace/CrabCLI --run-id git-integration-<commit> --crab-bin /Volumes/Workspace/crabbuild-target/crab-git-integration-production-hardening/release/crab --size-mib 64` | report status `passed`, no failed checks |
...
- `crab/src/git/fetch_transport.rs`
- `crab/src/git/push_native.rs`
- `crab/src/git/push.rs` for the live-discovered pre-acquired-lock release gap
  and publication of the already-defined `commit-graph-summary` shallow-fetch
  contract
- `crab/src/core/error.rs`
- `crab/src/core/error_catalog.rs`
- `crab/src/storage/retry.rs`
- `crates/crab-git/src/tag.rs`
- `crab/tests/remote_helper_transcript.rs`
- `crab/tests/e2e_add_commit_push.rs`
...
- `crab/docs/guides/clone.md`
- `crab/docs/design/technical-design.md`
- `plans/012-harden-crab-git-object-store-integration.md` for verified
  dependency-contract, scope, command-path, and completion-evidence corrections
- `plans/README.md` status only

The three cross-cutting error files are required only to preserve the strict
tag error's source chain, register its stable CLI code/category, and classify it
as non-retryable. They do not change an existing error's serialized shape.

**Out of scope — do not touch:**

- Manifest schema or paths, Git pack format, pointer syntax, xorb/shard format,
  or storage layout.
- Manifest CAS, distributed-lock acquisition/CAS semantics, staging-retirement,
  GC, or hydration algorithms except to preserve and test their existing
  contracts. The canonical push owner may release an installed lock on an
  early all-rejected outcome and publish the existing shallow-fetch summary.
...
- Suggested branch: `advisor/012-harden-crab-git-integration`.
...
```bash
CARGO_TARGET_DIR=/Volumes/Workspace/crabbuild-target/crab-git-integration-production-hardening \
...
rg -n '#\[ignore' crab/tests/remote_helper_transcript.rs crab/src/git/remote_helper.rs
```
...
### Step 3.2: Make tag-related helper options match Git's contract

Git's `gitremote-helpers` contract defines `option followtags true` as a fetch
hint: annotated tag objects should accompany objects whose targets are fetched.
It does not define an `include-tag` remote-helper option. Add a real-Git
in-memory integration case with an annotated remote tag pointing at a fetched
commit. Drive `option followtags true` and assert that complete-pack
installation materializes the tag object while the invoking Git process remains
the owner of local ref updates.

Reply `unsupported` to the undefined `include-tag` option. The public
`HelperOptions.include_tag` field exists in `v1.0.14`; leave it false and mark
it deprecated for removal in the next major version rather than removing it or
...
Change `collect_followtag_specs` to return a `Result` containing each synthesized
spec together with the tag-object SHA resolved by strict discovery. In
`run_native_push`:

- propagate base-manifest read errors instead of treating the remote as empty;
- propagate strict tag-discovery errors;
- treat strict ref/object resolution errors as fatal and carry the resolved SHA
  forward without a second time-of-check/time-of-use ref lookup;
...
Direct pack publication has included a verified repo-local `.idx` companion
since `v1.0.14`. Validate that companion through the canonical `PackStore`
download seam before fetching its pack; a missing or empty published index is
a torn/corrupt generation, not a legacy fallback. Keep the trait addition
backward-compatible with a default for generic non-object-store pack sources.
...
python3 crab/scripts/e2e/run_add_commit_push_rustfs_smoke.py \
  ...
  --crab-bin /Volumes/Workspace/crabbuild-target/crab-git-integration-production-hardening/release/crab \
  --size-mib 64
```
...
- [x] Only a missing routed manifest can produce an empty ref advertisement.
- [x] Corrupt/unreadable manifest tests make `git ls-remote` and clone fail.
- [x] `remote_helper_transcript` has zero ignored tests.
- [x] `v2_fetch_transport` has zero ignored tests and documents the explicit
      unsupported stateless-connect contract.
- [x] Helper capabilities do not advertise `filter`, `connect`, or
      `stateless-connect`.
- [x] `option filter blob:none` returns `unsupported`; direct library use with
...
- [x] Follow-tags discovery, manifest read, and tag resolution errors publish
      neither branch nor tag.
- [x] Successful follow-tags publishes branch and annotated tag through one
      existing manifest CAS.
- [x] Focused helper/fetch/push/tag tests pass.
- [x] `cargo fmt --all --check` passes.
- [x] Clippy passes for `crab` and `crab-git`, or any unrelated baseline failure
      is identified with a clean touched-file proof; no touched warning remains.
- [x] Manual 64 MiB RustFS report passes every success and expected-failure
      case with no credential disclosure.
- [x] User docs contain the exact support matrix and distinguish the two
      meanings of “protocol v2.”
- [x] No durable format, storage layout, config key, dependency, or second
      production fetch/push path was added.
- [x] `git diff --numstat` is reviewed; source growth is justified by deleted
      fallback/dead-option complexity and behavior-level tests.
- [x] No files outside the amended in-scope list are modified by this plan.
- [x] `plans/README.md` status is updated.

### Completion evidence

- Focused process tests: 52 passed, zero ignored.
- Git library slice: 669 passed, zero failed, one unrelated DynamoDB Local test
  ignored by its existing environment contract.
- Git-v2 contract: 5 passed, zero ignored; tag owner: 8 passed.
- RustFS: status `passed`, 72 checks, 189 real commands, no failed checks;
  release binary SHA-256
  `1df1a2b95809fc495e072a46c64ba48220808b90807ace21535d06742752cd32`.
- Exact all-target Clippy remains blocked by existing `origin/main` diagnostics,
  beginning at `crates/crab-workflow/src/template/context.rs:342`. `crab-git`
  library Clippy passes with `-D warnings`; scoped Crab library output contains
  no diagnostic in a changed hunk.
...
- A supported Git version documents or sends a remote-helper `include-tag`
  option contrary to the checked `gitremote-helpers` contract; report the
  version and contract before selecting new behavior.
