...
impl RemoteGitSnapshot {
    ...
    /// Find the newest first-parent commit that changed each listed directory entry.
    ///
    /// Entries must be immediate children returned by this snapshot. The result
    /// preserves their order and shares one bounded history walk across all entries.
    pub async fn latest_directory_entry_commits(
        &self,
        directory: &GitPath,
        entries: &[TreeEntry],
        operation: &OperationContext,
        ...
        for entry in entries {
            let name = entry.path.file_name().ok_or(Error::InternalInvariant {
                invariant: "directory listing contained the repository root",
            })?;
            if directory.join(name)? != entry.path {
                return Err(Error::InternalInvariant {
                    invariant: "directory history entry was not an immediate child",
                });
            }
        }
