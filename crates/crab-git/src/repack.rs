...
pub struct GeometricRepackedPack {
    ...
    pub is_new: bool,
    /// Sparse target-to-base identities for external `REF_DELTA` entries.
    pub external_delta_bases: BTreeMap<gix_hash::ObjectId, gix_hash::ObjectId>,
}
...
impl GeometricRepackedPack {
    ...
    /// Return external `REF_DELTA` target/base identities in pack-offset order.
    #[must_use]
    pub fn external_delta_bases(&self) -> &BTreeMap<gix_hash::ObjectId, gix_hash::ObjectId> {
...
pub fn incremental_repack_cut(weights: &[u64], factor: u64) -> usize {
    ...
    incremental_repack_cut_in_order(&weights, factor)
}

/// Return the smallest chronological suffix worth coalescing.
///
/// Unlike [`incremental_repack_cut`], this preserves the caller's source
/// order. Layered checkpoints use publication order so replacing a suffix
/// cannot reorder duplicate-object precedence while still applying the same
/// geometric promotion rule to compressed-byte weights.
#[must_use]
pub fn incremental_repack_cut_in_order(weights: &[u64], factor: u64) -> usize {
    if factor < 2 || weights.len() < 2 {
        return 0;
    }
...
fn repack_repository(
    ...
    for pack_path in pack_paths {
        ...
        packs.push(GeometricRepackedPack {
            ...
            external_delta_bases: BTreeMap::new(),
        });
    }
...
fn consolidate_pack_suffix_with_options(
    ...
    if source_objects_are_disjoint {
        ...
        let mut generated = verified_generated_pack(
            pack_path,
            GeneratedPackValidation::Structural,
            Some(&expected_oids),
            None,
        )?;
        if !delta_bases.is_empty() {
            generated.external_delta_bases = generated_external_delta_bases(
                generated.pack_path(),
    ...
    let mut generated = verified_generated_pack(pack_path, generated_validation, None, None)?;
    if !delta_bases.is_empty() {
        generated.external_delta_bases = generated_external_delta_bases(
            generated.pack_path(),
            generated.index_path(),
...
fn generated_external_delta_bases(
    pack_path: &Path,
    index_path: &Path,
    reverse_index_path: &Path,
    pack_size: u64,
) -> Result<BTreeMap<gix_hash::ObjectId, gix_hash::ObjectId>, RepackError> {
    let locations = PackLocationIter::open(index_path, reverse_index_path, pack_size)?;
    let mut object_ids_by_offset = BTreeMap::new();
    for location in locations {
        let location = location?;
        object_ids_by_offset.insert(location.pack_offset, location.oid);
    }
    let object_ids = object_ids_by_offset
        .values()
        .copied()
        .collect::<BTreeSet<_>>();
    ...
    .map_err(|error| RepackError::SourceIntegrity {
        pack_id: pack_path.display().to_string(),
        reason: format!("cannot scan generated pack entry headers: {error}"),
    })?;
    let mut external = BTreeMap::new();
    for entry in entries {
        let entry = entry.map_err(|error| RepackError::SourceIntegrity {
            pack_id: pack_path.display().to_string(),
            reason: format!("cannot scan generated pack entry: {error}"),
        })?;
        let gix_pack::data::entry::Header::RefDelta { base_id } = entry.header else {
            continue;
        };
        // Bases indexed by this pack are internal and need no source closure;
        // only absent identities must remain reachable from an earlier layer.
        if object_ids.contains(&base_id) {
            continue;
        }
        let target = object_ids_by_offset
            .get(&entry.pack_offset)
            .copied()
            .ok_or_else(|| RepackError::SourceIntegrity {
                pack_id: pack_path.display().to_string(),
                reason: format!(
                    "generated REF_DELTA entry at offset {} is absent from its index",
                    entry.pack_offset
                ),
            })?;
        if let Some(previous) = external.insert(target, base_id)
            && previous != base_id
        {
            return Err(RepackError::SourceIntegrity {
                pack_id: pack_path.display().to_string(),
                reason: format!("generated object {target} has conflicting REF_DELTA bases"),
            });
        }
    }
    Ok(external)
...
fn pin_refs_and_fsck(
    ...
    for (index, oid) in refs.iter().enumerate() {
        // A visible ref may legally target a tag, tree, or blob (for
        // example an annotated tag or a Git-generated namespace). Pinning
        // every object under `refs/heads` makes Git reject valid inventories
        // before fsck can verify them. Temporary tags accept every Git object
        // type while keeping the complete reachable closure visible.
        let name = format!("refs/tags/crab-repack-{index}");
        run_git(
            Command::new("git")
...
mod tests {
    ...
    #[test]
    fn ordered_incremental_cut_preserves_layer_precedence() {
        assert_eq!(incremental_repack_cut_in_order(&[900, 700, 9, 9], 2), 2);
        assert_eq!(incremental_repack_cut_in_order(&[900, 9], 2), 0);
        assert_eq!(incremental_repack_cut_in_order(&[900, 700, 400], 2), 2);
    }
    ...
    fn geometric_repack_preserves_complete_ref_object_graph() -> Result<(), RepackError> {
        ...
        let blob = git_output(
            Command::new("git")
                .arg("-C")
                .arg(&repository)
                .args(["rev-parse", "HEAD:first.txt"]),
            "resolve visible blob ref",
        )?;
        let refs = BTreeSet::from([tip, blob]);
    ...
    #[test]
    fn concatenate_complete_pack_inventory_closes_a_cross_pack_ref_delta() -> Result<(), RepackError>
    {
        let root =
            tempfile::tempdir().map_err(|source| io_error("create cross-pack root", source))?;
        ...
        let base_pack = crate::incoming_pack::IncomingPack::from_generated_objects(
        ...
        .map_err(|source| RepackError::SourceIntegrity {
            pack_id: "concat-base".to_owned(),
            reason: source.to_string(),
        })?
        .prepare(root.path(), 16 * 1024 * 1024, &cancelled)
        .map_err(|source| RepackError::SourceIntegrity {
            pack_id: "concat-base".to_owned(),
            reason: source.to_string(),
        })?
        ...
        let dependent_pack = crate::incoming_pack::IncomingPack::from_generated_objects(
        ...
        .map_err(|source| RepackError::SourceIntegrity {
            pack_id: "concat-dependent".to_owned(),
            reason: source.to_string(),
        })?
        ...
        let concatenated = concatenate_complete_pack_inventory(&sources)?;
        let repository = root.path().join("concatenated.git");
        initialize_bare_repository(&repository)?;
        let repository_arg = repository.to_str().ok_or_else(|| {
            io_error(
                "encode concatenated repository path",
                io::Error::new(io::ErrorKind::InvalidData, "path is not UTF-8"),
            )
        })?;
        git_with_stdin(
            ...
            &std::fs::read(concatenated.pack_path())
                .map_err(|source| io_error("read concatenated thin pack", source))?,
        );
    ...
    -> Result<(), RepackError> {
        ...
        let dependent_repacked = consolidate_committed_pack_suffix_with_delta_bases(
            &dependent_sources,
            &[RepackDeltaBase {
                oid: base_oid,
                kind: gix_object::Kind::Blob,
        ...
        assert_eq!(
            dependent.external_delta_bases().get(&target_oid),
            Some(&base_oid)
        );
        git_with_stdin(
            &[
