...
The normal catalog path reads bounded index sidecars and pack trailers. If a standard Git `.idx` or `.rev` sidecar is absent, maintenance downloads the immutable canonical pack once under the manifest size bound, verifies its Git SHA-1, Crab BLAKE3 identity, and object count, then rebuilds both sidecars with Gitoxide in a bounded temporary directory. Content-addressed storage publishes the regenerated bytes idempotently. A present but malformed sidecar still fails closed; the repair path never overwrites existing bytes and never synthesizes visibility proof from pack contents.
...
- **Visibility receipt reconstruction:** Missing journal or visibility evidence cannot yet be rebuilt from a durable verified receipt after restart. Standard Git locator sidecars are repaired from the verified canonical pack body during catalog maintenance; that repair does not replace the visibility proof.
