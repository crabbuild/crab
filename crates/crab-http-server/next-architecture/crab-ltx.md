...
[Design index](README.md) · Local/remote library and issue/comment/label/status HTTP integration implemented.
...
comment, label and commit-status commands publish LTX before success, cold activation restores the exact
...
| Server wiring | Repository issue/comment/label/status SQL, owner/control publication, scheduled owner-bound compaction, exact local-cut pruning, local/remote/idle/stale-owner routing, public HTTP response gating and source-loss restore are wired; remaining collaboration domains are not |
