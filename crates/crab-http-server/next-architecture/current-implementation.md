...
| Collaboration persistence | [app_storage.rs](../src/app_storage.rs), [cells/repository.rs](../src/cells/repository.rs) | Issues, comments, Labels, commit statuses, check runs, branch protections, repository lifecycle, Pull requests, Pull comments and reviews use transactional repository SQLite plus LTX. Releases remain on bounded JSON/CAS. No legacy application importer exists | Move releases directly to typed module APIs; old bucket data is manually deleted at cutover |
...
releases and other unfinished collaboration domains still use application JSON.
