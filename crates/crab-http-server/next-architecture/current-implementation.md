...
identity, issue/comment/label/status/check sequences and rows; integration tests prove replay,
...
public issue/comment/label/status/check routes now use that runtime and no longer read or write
...
`cells release migrations` cursor reports pending and failed Cells. The complete issue/comment/label/status/check HTTP route
...
pulls, releases, settings and other collaboration domains still use application JSON.
