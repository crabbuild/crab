    ...
    for (identity, access, permitted) in [
        ...
        *h.provider.mode.lock().await = identity.into();
        ...
        for (method, route, body) in [
            ...
            let response = h
                .http
                .request(method, format!("{}/git/team/private.git/{route}", h.origin))
...
async fn git_tokens_are_read_scoped_and_revoked_with_the_browser_session() {
    ...
    }
    {
        ...
        let request = axum::http::Request::builder()
            .method("POST")
            .uri("/git/team/private.git/git-upload-pack")
...
#[tokio::test]
async fn lfs_tokens_enforce_scope_and_revocation_before_upload_publication() {
    ...
    let cookie = h.login().await;
    let session = h.json("/api/session", &cookie).await;
    let batch_url = format!("{}/git/team/private.git/info/lfs/objects/batch", h.origin);
    let object_path = format!("/git/team/private.git/info/lfs/objects/{OID}?size=5");
    ...
    for access in ["read", "write"] {
        ...
        let response = h
            ...
            .body(json!({"operation":"upload","objects":[{"oid":OID,"size":5}]}).to_string())
            .send()
            .await
            .unwrap();
        assert_eq!(
        ...
        if access == "read" {
            ...
            assert_eq!(
                h.http
                    .put(format!("{}{object_path}", h.origin))
                    .basic_auth("crab", Some(token))
                    .body("hello")
                    .send()
                    .await
        ...
        let actions: Value = serde_json::from_slice(&response.bytes().await.unwrap()).unwrap();
        assert_eq!(
            actions["objects"][0]["actions"]["upload"]["href"],
            format!("{}{object_path}", h.origin)
        );
        let authorization = actions["objects"][0]["actions"]["upload"]["header"]["Authorization"]
            .as_str()
            .unwrap();
        assert_eq!(
            h.http
        ...
        let downloaded = h
            .http
            .get(format!("{}{object_path}", h.origin))
            .basic_auth("crab", read_token.as_deref())
            .send()
            .await
            .unwrap();
        assert_eq!(downloaded.status(), StatusCode::OK);
        assert_eq!(downloaded.bytes().await.unwrap(), "hello");
        let absent = format!(
            "/git/team/private.git/info/lfs/objects/{}?size=5",
            "0".repeat(64)
        );
        ...
        let body = Body::from_stream(futures_util::stream::once(async move {
            receive
                .await
                .map(|()| Bytes::from_static(b"hello"))
                .map_err(std::io::Error::other)
        }));
        let app = router(Arc::clone(&h.server));
        let request = Request::builder()
            .method("PUT")
            .uri(&absent)
            .header("host", h.origin.strip_prefix("http://").unwrap())
            .header(header::AUTHORIZATION, authorization)
            .body(body)
            .unwrap();
        let client = tokio::spawn(app.oneshot(request));
        tokio::time::timeout(Duration::from_secs(2), async {
            while h.server.receives.is_empty() {
    ...
    h.server.receives.close();
    h.server.receives.wait().await;
    h.close().await;
}
