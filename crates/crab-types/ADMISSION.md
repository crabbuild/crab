...
| `pointer::{Pointer, PointerParseError, VERSION_LINE, MAX_POINTER_SIZE, is_pointer, hex_encode}` | wire | The Crab pointer file format is consumed by CLI, SDK, LFS/read paths, and server helpers as a stable wire payload. Parsing helpers are deterministic and perform no I/O. |
