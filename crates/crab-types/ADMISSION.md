...
| `time::{now_rfc3339_millis, from_epoch_millis, from_system_time, TimestampError}` | helper | Shared timestamp formatting keeps persisted/wire timestamps stable. Checked formatting rejects pre-epoch or out-of-range input with typed errors; the shared SystemTime conversion serves VFS serialization and the wall-clock wrapper. Formatting performs no config, I/O, or runtime ownership. |
