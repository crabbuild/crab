//! Blob metadata and manifest SQL rows.
...
pub(super) fn blob_metadata(connection: &Connection, key: &[u8]) -> Result<Option<BlobMetadata>> {
    validate_key(key)?;
    connection
        .query_row(
