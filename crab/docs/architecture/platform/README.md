...
faults and proves both activations complete. Production capacity qualification remains
...
issue/comment/label/status/check/settings/Pull/Release route group now uses typed Cell commands and queries. The runtime
...
before immutable body reads. A byte-precise node budget shared by managed WAL,
captured-but-unpublished LTX, newly materialized sparse pages, and Git/LFS/Release
staging rejects work before local growth can exceed its two-thirds disk allocation.
Transfer staging also rechecks actual free space before reading request bodies.
Label create/edit/delete/list, issue-label validation, commit-status
...
canonical external side effects. The hard cut has no legacy importer;
...
endpoint without the matching live advertisement. The issue/comment/label/status/check/settings product
...
the internal issue/comment/label/status/check/settings operation set: create and update commands plus
...
constructs one repository router before readiness; every public issue/comment/label/status/check/settings
