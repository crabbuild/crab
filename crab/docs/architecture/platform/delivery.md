...
| [effects.rs](../../../../crates/crab-cell-runtime/src/effects.rs) | Stable source effect IDs/digests, bounded claim/lease/retry/delivery, target inbox dedup/savepoint isolation, actor/LTX publication, private Resolve, exact-root recovery and sender/inbox cleanup horizons | Add Workflow/Queue adapters, compiled codecs, authenticated peer delivery, source acknowledgement wiring and native supervision |
...
from the published inbox after exact-root restore. Add the private authenticated
DeliverEffect/ResolveEffect protocol, source acknowledgement path and native
delivery supervisor; browser callers cannot forge source effect identities.
