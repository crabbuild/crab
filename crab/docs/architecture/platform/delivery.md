...
| [registry.rs](../../../../crates/crab-cell-runtime/src/registry.rs), [maintenance.rs](../../../../crates/crab-cell-runtime/src/maintenance.rs), [activity_pool.rs](../../../../crates/crab-cell-runtime/src/activity_pool.rs) | Startup-only static module registration, bounded migration/schema/namespace/retained-code validation, exact next schema/code migration selection, current-plus-retained module inventory in the canonical release, descriptor/function/Workflow-definition/native-activity inventory matching, typed Command/Query trampolines, transaction-scoped compiled dispatch, async and fixed-pool blocking activity dispatch, blocking-namespace admission inventory and exact namespace/role/code/schema support checks; candidate startup and compatible activation compare the selected predecessor descriptor and reject removed or narrowed code/schema, codec, migration, Workflow, activity or namespace contracts; the same comparison selects a bounded role-aware persisted-work inventory over runtime, Queue and Workflow tables during maintenance; typed local and authenticated remote dispatch reuse these exact bindings | Add purpose-built transforms for removals whose source pair or namespace is absent from the candidate |
...
| [client.rs](../../../../crates/crab-cell-runtime/src/client.rs) | Typed local and authenticated peer command/query/Resolve capabilities, namespace/code/schema/incarnation checks, canonical operation digest, outcome classification and minimum receipts; all repository collaboration routes and typed KV, SQL, Queue and Workflow handles use it | Add workload-specific primitive consumers |
| [HTTP server.rs](../../../../crates/crab-http-server/src/server.rs), [local_disk.rs](../../../../crates/crab-http-server/src/local_disk.rs), [issues.rs](../../../../crates/crab-http-server/src/issues.rs), [labels.rs](../../../../crates/crab-http-server/src/labels.rs), [statuses.rs](../../../../crates/crab-http-server/src/statuses.rs), [checks.rs](../../../../crates/crab-http-server/src/checks.rs), [pulls.rs](../../../../crates/crab-http-server/src/pulls.rs), [releases.rs](../../../../crates/crab-http-server/src/releases.rs), [repository_settings.rs](../../../../crates/crab-http-server/src/repository_settings.rs), [peer.rs](../../../../crates/crab-http-server/src/peer.rs), [peer_tls.rs](../../../../crates/crab-http-server/src/peer_tls.rs), [cells.rs](../../../../crates/crab-http-server/src/cells.rs), [cells/initializer.rs](../../../../crates/crab-http-server/src/cells/initializer.rs), [cells/repository.rs](../../../../crates/crab-http-server/src/cells/repository.rs), [cells/router.rs](../../../../crates/crab-http-server/src/cells/router.rs), [cells/scheduler.rs](../../../../crates/crab-http-server/src/cells/scheduler.rs) | Static repository registry; typed issue/comment/label/status/check/settings/Pull/Release bindings; permanent submission replay; verified create/adopt empty-Cell initialization; startup/root gates; mTLS owner routing; stale-owner takeover; bounded scheduler, release migration, maintenance and shutdown; resource-derived page-cache/native/descriptor Cell admission, shared CPU/memory-derived dirty-job admission, byte-weighted full-job scratch admission, and one byte-precise budget shared by active WAL, retained LTX, sparse pages and same-volume Git/LFS/Release staging with transfer-time actual-free-space checks. Pull merge and Release publication intent are durable in SQLite while exact Git publication and immutable Release-asset bytes remain canonical external effects. Catalog v1 and legacy application import are deliberately unsupported | Add unsupported-source/namespace maintenance transforms and real multi-Pod fault qualification |
...
`CellReplica::prepare_scheduled_compaction` selects at most 128 contiguous inputs,
promotes eight or more preceding-level inputs, and falls back to a complete
level-nine replacement before segment or graph-byte admission is exhausted.
...
The publisher now runs scheduled representation compaction before an append,
renews ownership during the operation, publishes each replacement through the
ordinary authority CAS, and preserves the durable due summary. An exact
segment/graph-byte admission rejection forces one full replacement and one retry
of the already captured batch; it never reruns SQL. Exact bootstrap,
command and migration confirmation calls `ManagedDb::prune_captured`; local
capture accounting is released only after path and manifest equality are
reverified. `dispatcher_compacts_before_segment_admission_is_exhausted` executes
ten mutations with a four-segment ceiling and proves both forward progress and
the exact restored value.
...
session restores and reads the same row. Authorized issue/comment/label/status/check/settings handlers
...
and exercised by the public issue/comment/label/status/check/settings adapters. The server acceptance test
...
typed codecs and bindings for the internal issue/comment/label/status/check/settings route group.
Its runtime test
...
| tests/scheduler.rs | drop every wake notification, evict all due Cells, restart scanner and observe eventual published progress; activating release migrates an idle retained Cell and records terminal progress before ready |
...
| tests/migration.rs | digest conflict leaves authority unchanged; retained-code adjacent migration replaces capability, publishes schema/root/code and restores from that exact root after source loss; same-schema code-only migration publishes a system cut without a migration-ledger row; authenticated peer migration derives its local plan and reconciles an exact retry |
...
capability. The schema path also restores after local-source loss. The server now
walks rendezvous-assigned catalog shards during activation, uses the ordinary
local/remote/idle/takeover router, limits migration to 16 jobs per node and one
per Cell, and stores monotonic terminal progress with conditional writes. The
peer request carries versions rather than SQL; the receiver independently
derives the exact registry successor. The administrative status command derives
pending Cells from catalog/control and pages terminal failures. Maintenance now
drains the advertised fleet, acquires an operation-keyed signed singleton lease,
runs the same exact-root migration path sequentially in a local-only one-worker
runtime, performs conservative runtime/Queue/Workflow persisted-work admission
for a breaking descriptor, shuts it down, proves exclusive directory ownership
plus current inventory, publishes Ready and withdraws the lease. Continue with
arbitrary maintenance transforms and real fleet qualification,
reusing the existing image build, storage/auth construction, health routes and
metrics exporter. Both activation strategies refuse the final Ready CAS while
any non-tombstoned Cell remains on a retained predecessor pair.

`migration_digest_conflict_blocks_activation`,
`migration_replaces_capability_publishes_schema_and_restores_exact_root`, and
`code_only_migration_publishes_new_code_without_schema_ledger_entry`, and
`authenticated_peer_migration_derives_plan_and_reconciles_retry` are implemented
in `crates/crab-cell-runtime/tests/migration.rs`. The HTTP scheduler additionally
proves an activating release migrates an idle Cell and records completion, while
the maintenance command test restores an old-code Cell from its LTX root,
migrates it offline and publishes Ready. Add
...
Hard-cutover acceptance proves there are no `app_storage` callers and inventories every retired
