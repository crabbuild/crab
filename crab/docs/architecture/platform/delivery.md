...
| [node.rs](../../../../crates/crab-cell-runtime/src/node.rs) | Canonical signed 15-second node advertisements, strict-create/ETag refresh with exact ambiguous-write reconciliation, fleet/certificate/release/key/inventory/capacity binding, monotonic progress, certificate-SPKI-bound session verification and streaming bounded live-fleet enumeration that fails closed on malformed/foreign active records | Add scheduler progress aggregation and stale-record collection |
...
identities. Workflow actions now use one command-scoped effect batch. The server
repository scheduler polls due Cells and runs the supervisor through internal
fleet/session-bound authorization. Add Queue dead-letter effect insertion and
generic non-repository polling.
...
runs one effect supervision step. Node-progress advertisements, 15-second
fallback, bounded retry queues and generic Workflow activity orchestration remain.
