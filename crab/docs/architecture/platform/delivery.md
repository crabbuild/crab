...
| [registry.rs](../../../../crates/crab-cell-runtime/src/registry.rs), [activity_pool.rs](../../../../crates/crab-cell-runtime/src/activity_pool.rs) | Startup-only static module registration, bounded migration/schema/namespace validation, exact next-migration selection for one executable code/schema pair, descriptor/function/Workflow-definition/native-activity inventory matching, canonical module/release digests, typed Command/Query trampolines, transaction-scoped compiled dispatch, async and fixed-pool blocking activity dispatch, blocking-namespace admission inventory and exact namespace/role/code/schema support checks; authenticated remote dispatch reuses these exact bindings | Add retained predecessor-code inventory and retention gates before removing an authoritative command/query codec or Workflow definition |
| [application.rs](../../../../crates/crab-cell-runtime/src/application.rs), [release.rs](../../../../crates/crab-cell-runtime/src/release.rs) | Canonical immutable root identity, exact-winner initialization, immutable descriptor upload, expected-revision prepared CAS, verified descriptor reads, operation-bound resumable activating/ready transitions, explicit 1–10,000 live-compatible-node activation quorum and release-aware catalog provisioning with post-publication operation recheck | Add catalog-wide Cell migration progress, predecessor-code/code-only rollover and maintenance activation |
...
| tests/migration.rs | digest conflict leaves authority unchanged; successful adjacent migration replaces capability, publishes schema/root/code and restores from that exact root after source loss |
...
release operation after publication. The runtime now completes one
registry-selected adjacent schema migration: it closes the source capability,
commits migration ledger and metadata on the fixed worker, publishes the LTX
root/schema/code tuple atomically, returns a new capability and restores the
result after local-source loss. Continue with catalog-wide migration progress,
retained predecessor-code and code-only rollover, and maintenance/drain
procedures from deployment.md, reusing the existing image build, storage/auth
construction, health routes and metrics exporter.

`migration_digest_conflict_blocks_activation` and
`migration_replaces_capability_publishes_schema_and_restores_exact_root` are
