...
a `JoinSet`, so a long native future does not stop catalog progress; any effect step follows the
activity, while an effect-only namespace runs inline. Scheduler cancellation
aborts and joins those jobs, which drops the supervisor and signals cooperative
activity cancellation. Exact registered IDs/codecs select the internal action
...
primitive deadlines. Durable node retry queues,
cross-Cell/per-namespace admission fairness and multi-node activity failure
...
release operation after publication. Continue with migration and
maintenance/drain procedures from deployment.md, reusing the existing
