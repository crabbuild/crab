...
| [effects.rs](../../../../crates/crab-cell-runtime/src/effects.rs) | Stable source effect IDs/digests, bounded claim/lease/retry/delivery, target inbox dedup/savepoint isolation, actor/LTX publication, authenticated generic peer delivery/Resolve, exact-root recovery and sender/inbox cleanup horizons | Add Workflow/Queue adapters and codecs, source claim/ack orchestration and native supervision |
...
from the published inbox after exact-root restore. Add Workflow/Queue effect
codecs, source claim/ack orchestration and the native delivery supervisor;
browser callers cannot forge source effect identities. The generic compiled
...
Implement catalog scanner and Tick through the normal actor command loop.
...
| tests/client.rs | signed generic effect delivery executes once across retries and private Resolve returns the durable inbox outcome |
