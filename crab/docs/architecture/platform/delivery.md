...
| [HTTP server.rs](../../../../crates/crab-http-server/src/server.rs), [peer.rs](../../../../crates/crab-http-server/src/peer.rs), [peer_tls.rs](../../../../crates/crab-http-server/src/peer_tls.rs), [cells.rs](../../../../crates/crab-http-server/src/cells.rs), [cells/repository.rs](../../../../crates/crab-http-server/src/cells/repository.rs), [cells/router.rs](../../../../crates/crab-http-server/src/cells/router.rs), [cells/importer.rs](../../../../crates/crab-http-server/src/cells/importer.rs) | Static repository registry with UUID lookup; typed issue/comment bindings; bounded runtime replay plus permanent product-submission replay/conflict and incomplete-reservation repair; durable rejection and source-loss restore; process-session runtime lifecycle; authoritative local peer resolution; current repository issuer/member/action reauthorization; strict Ed25519 certificate/key/CA loading; mandatory management mTLS; initial node publication and heartbeat; authoritative outbound owner reload; endpoint/enrollment equality; pinned bounded peer client reuse; one definitely-not-started stale-owner retry; release-fenced provision, rootless bootstrap, exact local reuse, authenticated remote selection and idle exact-root restore; idempotent first-install bootstrap plus descriptor/inventory startup gate before listener binding; maintenance-only, two-pass verified and resumable legacy issue/comment import | Add remaining collaboration-domain import, product adapters, active-owner takeover, migration/node eligibility quorum, complete resource-derived Cell budgets and hard-cut the authorized route group |
...
release bytes remain deterministic. `route_provisions_reuses_and_restores_repository_cell`
