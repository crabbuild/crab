...
| [registry.rs](../../../../crates/crab-cell-runtime/src/registry.rs) | Startup-only static module registration, migration/schema/namespace validation, exact descriptor/function/Workflow-definition/native-activity inventory matching, canonical module/release digests, typed Command/Query trampolines, transaction-scoped compiled dispatch and async activity dispatch with registry-owned primitive shard counts | Add compatibility activation and remote dispatch support |
...
| [HTTP server.rs](../../../../crates/crab-http-server/src/server.rs), [cells.rs](../../../../crates/crab-http-server/src/cells.rs), [cells/repository.rs](../../../../crates/crab-http-server/src/cells/repository.rs) | Static repository registry; typed issue/comment bindings; exact replay, durable rejection and source-loss restore; one process-session runtime participates in server readiness and ordered shutdown | Add release/current gate, resource-derived local Cell configuration, private routing and hard-cut the authorized product routes |
