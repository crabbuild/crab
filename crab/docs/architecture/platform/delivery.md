...
| [workflow.rs](../../../../crates/crab-cell-runtime/src/workflow.rs) | Normative schema, pinned and registry-verified definitions, deterministic start/signal/timer transitions, cancellation, bounded action persistence, published activity claims, leases, retry/completion and terminal cleanup plus typed workflow-ID-sharded start/signal/cancel/state dispatch | Add effects, native activity supervision and scheduler polling |
...
| [registry.rs](../../../../crates/crab-cell-runtime/src/registry.rs) | Startup-only static module registration, migration/schema/namespace validation, exact descriptor/function/Workflow-definition inventory matching, canonical module/release digests, typed Command/Query trampolines and transaction-scoped compiled dispatch with registry-owned primitive shard counts | Add server composition/release inspection |
