...
| [crab-cell-runtime](../../../../crates/crab-cell-runtime/src/lib.rs) | Stable IDs/control authority/schema, verified CAS catalog, worker-owned bootstrap, exact-root sparse activation, fixed SQL workers, ordered bounded reads, FIFO publication, retry, unknown outcomes, five-second SQL/native watchdog, bounded owner renewal, idle acquisition, observed takeover, drain and bounded authorized SQL/KV/Queue plus Workflow transition mechanics | Add sparse page-I/O deadlines, automatic fenced recovery, typed registry adapters and Workflow activity/effect/scheduler mechanics |
...
| [workflow.rs](../../../../crates/crab-cell-runtime/src/workflow.rs) | Normative schema, pinned definitions, deterministic start/signal/timer transitions, cancellation and bounded action persistence | Add activity claim/lease/completion, effects, terminal cleanup, registry dispatch and scheduler polling |
