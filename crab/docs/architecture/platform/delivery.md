...
| [effects.rs](../../../../crates/crab-cell-runtime/src/effects.rs) | Stable source effect IDs/digests, owner-independent typed command intentions, delivery-time destination Describe, typed registered claim/validation/lease operations, one-effect native supervision, target inbox dedup/savepoint isolation, actor/LTX publication, authenticated generic peer delivery/Resolve, exact-root recovery and sender/inbox cleanup horizons; Queue DLQ uses the typed path and the server repository scanner runs one supervisor step for a due source | Migrate legacy raw Workflow effects and add generic non-repository polling |
...
Workflow actions use one command-scoped effect batch but still need migration
from raw pre-encoded operations to typed command intentions. The server
