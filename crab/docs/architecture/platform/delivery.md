...
| [effects.rs](../../../../crates/crab-cell-runtime/src/effects.rs) | Stable source effect IDs/digests, command-scoped Workflow effect insertion, typed registered claim/validation/lease operations, one-effect native supervision, target inbox dedup/savepoint isolation, actor/LTX publication, authenticated generic peer delivery/Resolve, exact-root recovery and sender/inbox cleanup horizons | Add Queue dead-letter effects and catalog-driven node polling |
...
identities. Workflow actions now use one command-scoped effect batch; add Queue
dead-letter effect insertion and catalog-driven node polling.
