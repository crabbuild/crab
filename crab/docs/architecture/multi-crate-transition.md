...
| `crab-lfs-server` | Standard Git LFS HTTP composition: discovery, Batch/basic transfers, verify actions, File Locking, authentication, repository policy, limits, upload spooling, TLS/listener setup, and shipped gateway binary | Reusable object layout/integrity, provider SDK construction, CLI transfer-agent protocol, Crab-native remote-helper behavior, or cache-server routes | `make architecture-check` keeps the server isolated with only `crab-lfs` and `crab-storage` internal normal dependencies; HTTP qualification with an unmodified Git LFS client is the release gate |
