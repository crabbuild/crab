...
- Name precision matters here: the current Rust `crab-auth-server` package is
  ...
  endpoint remains the Python FastAPI deployment under `crab/deploy/auth`,
  ...
  replaces or fully mirrors the `crab/deploy/auth` contract with endpoint
...
1. Done: finish the SDK local-worktree selector migration. Default and
   `credentialed-auth` SDK builds support static-env-compatible `.crab/remote`
...
- `crab-sdk` paid down the broad default consumer edge. It already imports
  ...
  Local workspace `.crab/remote` values that are raw cloud or `crab://` URLs now
...
| `cargo tree -p crab-cache --edges normal --depth 1` | Depends on `blake3`, `bytes`, `crab-types`, `crab-xet`, `globset`, serde/JSON, `thiserror`, `toml`, and `tracing`; no `rusqlite`, `filetime`, `tokio`, direct `reqwest v0.12`, `axum`, `object_store`, or `crab-cache-server` | Default `crab-cache` is a client/shared Module with cache keys, route taxonomy, prefetch policy, and pure cache-service DTOs, not SQLite-backed local persistence, a cache-service HTTP Adapter, server runtime, or storage Adapter |
...
- `crab-cache` now owns the shared cache route taxonomy used by both
  ...
  construction, PEM CA bundle validation, and the `.crab/prefetch.toml`
...
- `crab-sdk` now loads `.crab/prefetch.toml` through `crab-cache` and resolves
...
- `.crab/prefetch.toml` parsing has moved because prefetch profiles are a
...
| Import/export/restripe/replication | `crab/src/import`, `crab/src/restripe`, `crab/src/replication` | Treat as orchestration Modules. Move lower reusable pieces into storage/metadata/coordination first; leave command workflows in CLI or a later orchestration crate. |
...
| `crab-cache` | default route taxonomy, route-to-local-cache-key derivation through `cache_key_for_path`, `CacheKey`, prefetch-profile parser/lookup/error tests, pure cache-service DTO tests, `LocalCache` tests under `local-cache`, remote-client tests with mocked HTTP under `remote-client`, range-cache handle tests under `xet-chunk-cache`, dependency proof excluding SQLite/local-cache persistence, direct cache-service HTTP, server runtimes, storage transport, and CLI crates from the default build; `make architecture-check` source-scope proof excluding CLI config/errors/output, auth/storage/read/metadata/Git/LFS/workflow/coordination/SDK/server crates, object-store/provider construction, provider SDKs, direct upstream `xet-data`/`xet-core-structures`, SlateDB, command stdio, and direct `xet-client`/`xet-runtime` imports outside `xet_chunk_cache.rs` | cache-accelerated hydrate/push smoke |
...
| Crab Auth endpoint deployment | `cd crab/deploy/auth && python -m pytest tests`; `cd crab && make auth-helper-packaging-check`; endpoint/helper contract fixtures proving `/v1/credentials`, protected-push prepare/finalize, ready/doctor, receive-helper JSON, and view-helper JSON shapes stay compatible with `crab-auth` parsers and `crab-auth-server` helper output | Path-ACL local/RustFS smoke through `crab/deploy/auth/scripts/e2e-path-acl-local.py` or `crab/deploy/auth/scripts/e2e-path-acl-rustfs.py` before auth endpoint releases |
