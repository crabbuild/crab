...
The [Cellule](https://github.com/crabbuild/cellule) framework owns identity, authority, SQLite execution, LTX publication, primitives, release control, and private peers. Crab owns the repository adapters and product qualification. Production qualification still requires measured capacity and real multi-Pod network fault evidence.
...
Set a worktree-specific external Cargo target directory before every Rust command. Resolve `CELLULE_MANIFEST` from Crab's pinned dependency before running Cellule commands; run the Crab server commands from this checkout.
...
CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/cellule-pinned \
  cargo test --manifest-path "$CELLULE_MANIFEST" -p cellule-runtime

CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/cellule-pinned \
  cargo test --manifest-path "$CELLULE_MANIFEST" -p cellule-ltx --features replica
...
```bash
CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/cellule-pinned \
  cargo clippy --manifest-path "$CELLULE_MANIFEST" -p cellule-ltx -p cellule-runtime \
  --all-targets -- -D warnings

CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/crab-current \
  cargo clippy -p crab-http-server --all-targets -- -D warnings
```

Use stable target suffixes unique to each checkout.
...
CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/crab-main \
  cargo run --manifest-path "$CELLULE_MANIFEST" -p cellule-runtime --bin qualification_receipt --locked -- \
...
CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/crab-rustfs \
  cargo test --manifest-path "$CELLULE_MANIFEST" -p cellule-ltx --features replica --test cell_roots \
...
CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/crab-rustfs \
  cargo test --manifest-path "$CELLULE_MANIFEST" -p cellule-runtime --test actor \
...
CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/crab-rustfs \
  cargo test --manifest-path "$CELLULE_MANIFEST" -p cellule-runtime --lib \
...
CRAB_COORDINATION_SEED=41 CRAB_COORDINATION_STEPS=256 \
  CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/crab-main \
  cargo test --manifest-path "$CELLULE_MANIFEST" -p cellule-runtime coordination_sim::replay_requested_seed_from_environment \
...
`cellule-ltx` evidence must cover:
...
The credential-free filesystem fixture covers three owner-loss boundaries:
before primitive writes, after leases, and after acknowledged settlements.
It checks the successor and independent observer through typed application
handles, but does not produce provider or release evidence:

```bash
CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/crab-current \
  cargo test -p crab-http-server --test public_cell_process_fault \
  filesystem_owner_kill_ --locked -- --nocapture
```
...
CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/crab-qualification-receipts \
  cargo test --manifest-path "$CELLULE_MANIFEST" -p cellule-runtime qualification --lib --locked
```
