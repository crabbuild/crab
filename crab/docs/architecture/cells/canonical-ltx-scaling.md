...
| --- | --- |
| Request entry | [`RepositoryCellRouter::route_target`](../../../../crates/crab-http-server/src/cells/router.rs) calls `route_existing` twice around an activation lock, then repeats catalog and control loads for activation. |
| Metadata lookup | `CellCatalog::lookup` (`cellule-runtime/src/catalog.rs`) loads the shard head and every referenced immutable catalog page; `CellAuthority::load` (`cellule-runtime/src/authority.rs`) separately reads exact control. |
| Local residency | `CellRuntime::resident_handle` (`cellule-runtime/src/actor.rs`) asks the actor for a fully resident owner before remote metadata; `local_handle` (`cellule-runtime/src/actor.rs`) remains the verified slow-path lookup for sparse or activation callers. Fenced, draining, and non-resident actors miss safely. |
...
Add a TLA+ model under Cellule's `crates/cellule-runtime/model/` for two Cells, up to
...
Resolve Cellule's manifest from Crab's pinned dependency. Use a distinct external Cargo target directory for each checkout.
...
CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/cellule-pinned \
  cargo test --manifest-path "$CELLULE_MANIFEST" -p cellule-ltx --locked

CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/cellule-pinned \
  cargo test --manifest-path "$CELLULE_MANIFEST" -p cellule-ltx --features replica --locked

CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/cellule-pinned \
  cargo test --manifest-path "$CELLULE_MANIFEST" -p cellule-runtime --locked

CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/crab-current \
  cargo test -p crab-http-server --locked

CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/cellule-pinned \
  cargo clippy --manifest-path "$CELLULE_MANIFEST" -p cellule-ltx -p cellule-runtime --all-targets --locked -- -D warnings

CARGO_TARGET_DIR=$HOME/Workspace/crabbuild-target/crab-current \
  cargo clippy -p crab-http-server --all-targets --locked -- -D warnings

node crab/docs/architecture/cells/validate.mjs
```
