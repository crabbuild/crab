...
`crab repack` merges multiple small pack files in the remote store into fewer,
larger packs. This reduces the number of S3 objects, improves listing
performance, and can reduce storage costs by eliminating per-object overhead.

Over time, as pushes create new pack files, the number of packs can grow. Repack
consolidates them while preserving all reachable Git objects and atomically
...
3. Verifies each source pair, then streams `git pack-objects` output to disk.
4. Builds and verifies the replacement `.idx` and `.rev`, then runs `git fsck`
   against a validation repository containing only the replacement pack.
5. Uploads the three immutable files and a compacted pack-index segment.
...
The replacement inventory records the complete pinned manifest ref-tip set.
Its object count comes from the verified Git index rather than an estimate.
...
- The `repack_auto_threshold` config option can trigger automatic repacking
  when the pack count exceeds a threshold.
...
|-----|---------|-------------|
| `repack_auto_threshold` | | Number of packs that triggers auto-repack |
...
{"schema":"repack.event","version":"1.0","timestamp":"2026-04-24T18:32:21.400Z","type":"result","data":{"packs_before":15,"packs_after":1,"bytes_before":2400000000,"bytes_after":2300000000,"duration_ms":4200}}
```
