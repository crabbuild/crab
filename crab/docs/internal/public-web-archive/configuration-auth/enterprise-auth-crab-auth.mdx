...
| `operation` | string | Concrete non-push Crab operation such as `fetch`, `clone`, `diff`, or `workflow-cache-pull`; request operation `"*"` and `push` are rejected |
...
Use `"*"` only inside policy rules. The CLI request always sends a concrete
operation.
...
| `"azure"` | `sas_token` or `bearer_token` | `MicrosoftAzureBuilder` |

The `region` field for AWS defaults to `us-east-1` if not provided.

### Provider scoping requirements

| Provider | Scoping requirement |
|----------|---------------------|
| AWS | Return STS credentials constrained by an inline session policy for the requested bucket prefix. |
| Azure | Use directory-scoped SAS (`sr=d` with `sdd`) for the requested repo prefix. Reject container-root repos. |
| GCP | Use Cloud Storage Credential Access Boundaries to downscope credentials to the requested repo prefix. |
...
jwks_client = PyJWKClient("https://login.corp.example.com/.well-known/jwks.json")
...
def vend_credentials():
    ...
    # 1. Validate the ID token (verify signature against IdP's JWKS)
    try:
...
- [Reference Implementation](https://github.com/crab-build/crab/tree/main/crab/deploy/auth) — deployable Crab Auth endpoint with Docker, Terraform, SAM, and Cloud Run support
...
- [Self-Hosted Auth Server](/docs/cli/authentication/self-hosted-auth-server)
