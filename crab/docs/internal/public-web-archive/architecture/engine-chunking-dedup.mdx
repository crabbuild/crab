...
Source: `crab/src/engine/chunker.rs`
...
![Three-tier dedup classification — Class A (existing on remote, skip), Class B (staged locally, needs upload), Class C (new, needs staging and upload). Classification cascades from cheapest to most expensive check.](./dedup-tiers.svg)
...
Source: `crab/src/engine/packer.rs`, `crab/src/engine/stream_packer.rs`

### CDC-Xorb Combined Pipeline

The `cdc_xorb` module combines CDC chunking and xorb building into a single
streaming pipeline, avoiding intermediate buffering:

```
File bytes → GearhashChunker → XorbBuilder → Xorb bytes
```

Source: `crab/src/engine/cdc_xorb.rs`
...
![Segment file format — frames are appended sequentially, each containing a length prefix, chunk data, and CRC32 checksum.](./segment-format.svg)
