...
Crab's workflow engine has strong Unix-first capabilities for production
workloads: parallel execution, crash recovery, retry, resource scheduling, and
structured observability. Those strengths do not make it a general DVC
replacement yet; migration, checkpoint lineage, artifact lifecycle, provider
compatibility, and release qualification remain explicit gates.

For users migrating from DVC, the `crab.yaml` schema is intentionally similar
in its supported subset, but migration is an inventory-and-verification
protocol rather than a file rename. Unsupported fields and remote/provider
semantics fail closed; `checkpoint: true` is not rewritten as `persist: true`,
and `.dvc/` remains the recovery source until the cutover report proves a
fresh-clone byte/tree/mode restore.
