...
│  ├── file-index/{h[:2]}/{h}  immutable file→shard map   │
...
- **SQLite index** (`index.db`) — the metadata backbone with five tables:
...
- `stage_chunk_for_file()` — write a chunk to the current segment + index
...
#### PersistentChunkIndex (`chunk-index.redb`) — NOT YET WIRED

A redb-backed persistent dedup index. Designed to live at
`~/.cache/crab/chunk-index.redb` so that dedup is immediately effective
across sessions without rebuilding from shards. Three tables:

- `chunks_v1`: chunk_hash (32 bytes) → xorb_hash (32 bytes) + chunk_index (4 bytes LE) + uncompressed_size (4 bytes LE)
- `shards_v1`: shard_hash (32 bytes) → empty value (presence marker, avoids re-installing)
- `meta_v1`: string key → string value (schema version)

Each shard install is a single redb write transaction for atomicity. Schema
mismatches trigger a full rebuild from the shard cache.

**Current status:** Implemented and tested but not yet wired into the push
pipeline. The push path currently rebuilds the in-memory `ChunkIndex` from
cached shards on every push (step 3). Wiring `PersistentChunkIndex` into
step 3 would eliminate the shard-parsing overhead on subsequent pushes —
the index would be read directly from redb instead of re-parsing all cached
shard files. This is a planned optimization.
...
| File-index entries | `CacheKey::FileIndex(MerkleHash)` | 32–64 bytes | LRU by mtime |
...
directory contains metadata fetched from S3 (shards, file-index entries,
manifests) that accelerates subsequent operations against the same repo.

**Important limitation:** The current cache has no per-repo namespacing.
Shards are content-addressed so cross-repo sharing is safe (same hash =
same bytes). However, file-index entries are repo-specific (the same
file_hash could map to different shard_hashes in different repos). In
practice this is not a problem for single-repo workflows, but users
working with multiple crab repos pointing to different S3 prefixes
could see stale file-index cache hits. A future improvement would
namespace the cache by `{bucket}/{prefix}` to eliminate this.
...
- `GET /health` — health check (2-second timeout)
...
The remote cache client is only enabled when configured AND the `/health`
...
For immutable paths (`/xorbs/`, `/shards/`, `/file-index/`):

```
1. Local disk cache lookup (hash-verified)
   ↓ miss
...
- **Immutable**: `/xorbs/`, `/shards/`, `/file-index/`, `/packs/`
- **Mutable**: everything else (refs, manifests, config, locks)

`path_to_cache_key()` maps immutable S3 paths to `CacheKey` variants:

- `repo/shards/{hash}` → `CacheKey::Shard(hash)`
- `repo/file-index/{hash}` → `CacheKey::FileIndex(hash)`
- Xorbs are intentionally excluded (64–128 MiB each; cached at chunk level)
...
- **Class A (Existing):** Already stored remotely. Found in the `ChunkIndex`
  (populated from shards in step 3) or reported as known by the remote cache
  service's dedup query. These chunks are skipped entirely — no packing, no
  upload.
...
If CachingStore has remote cache service:
    ...
    Chunks reported as "known" → class A (skip upload)
    ↓
...
Xorbs are uploaded to S3 via multipart (> 8 MiB) or single PUT. Xorb data
is consumed (moved) during upload — not cached locally due to size (64–128
MiB each). Chunk-level caching happens later during smudge/hydrate.

**Step 9: Upload shards + file-index entries**

```
Upload shards to S3
Upload file-index entries to S3
    ↓
Cache warming:
    Write each shard to LocalCache (CacheKey::Shard)
    Write each file-index entry to LocalCache (CacheKey::FileIndex)
    PUT shards to remote cache service (push warming, when enabled)
```

This means the very next `hydrate` or `diff` after a push finds all metadata
locally — zero S3 round-trips for shards and file-index lookups.
...
Parse pointer → file_hash, size, shard_hint
    ↓
Resolve shard:
    file_index_resolver.resolve_file_index(file_hash, shard_hint)
        → CachingStore.get_with_etag("file-index/{hash}")
            → LocalCache hit? return cached shard_hash
...
resolve_file_index(file_hash)
    → LocalCache.get_or_fetch(CacheKey::FileIndex(file_hash), fetch_from_s3)
    ↓
...
TermResolver:
    resolve_file_index(file_hash)
        → LocalCache.get_or_fetch(CacheKey::FileIndex(file_hash), ...)
    ↓
    get_or_download_shard(shard_hash)
...
service_mode = "cache_and_dedup"   # cache | dedup | cache_and_dedup
push_warming = true
```
...
the cache fits within configured budgets.
...
1. **Hash verification on every read.** Local cache entries are verified via
   `compute_data_hash` (shards, file-index) or blake3 (chunks) on every
   access. Corrupt entries are evicted and re-fetched transparently.
...
3. **Immutable objects only.** Only content-addressed, immutable objects
   (shards, file-index entries, chunks) are cached. Mutable objects (refs,
...
| `engine/staging/mod.rs` | `StagingArea` — local chunk store (SQLite + segments) |
| `engine/staging/index.rs` | SQLite index: files, segments, chunks, pending_chunks tables |
| `engine/staging/segment.rs` | Append-only segment files for compressed chunk data |
...
| `cmd/hydrate.rs` | `ShardHydrator` with `LocalCache` for shards + file-index |
| `diff/term_resolver.rs` | `TermResolver` with `LocalCache` for file-index + shards |
| `metadata/shard_sync.rs` | `ShardSynchronizer` — syncs shards into `LocalCache` |
| `metadata/shard_bloom.rs` | `ShardBloom` — per-shard bloom filter for chunk/file hashes |
| `metadata/chunk_index.rs` | `ChunkIndex` — in-memory chunk→xorb dedup index |
| `metadata/persistent_chunk_index.rs` | `PersistentChunkIndex` — redb-backed (not yet wired into push) |
