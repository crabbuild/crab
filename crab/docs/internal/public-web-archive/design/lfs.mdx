...
| Command | Description |
|---------|-------------|
| `crab lfs prune [--verify-remote] [--dry-run] [--force]` | Remove unreferenced LFS objects |
...
**GC safety:** LFS objects are never deleted by the push pipeline. Only
`crab lfs prune` deletes unreferenced objects, and only after the
