...
1. **Partial clones.** Git 2.30+ supports `--filter=blob:none`. Our remote helper should respect this and only fetch blob packs on demand. Interaction with smudge filter needs design.
