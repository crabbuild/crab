...
For repos where this is too large, crab offers an on-disk index option using `redb` (a Rust embedded key-value store). Lookup latency increases from ~100 ns to ~10 µs but memory drops to constant.
...
│   │   └── shard-list.json
│   └── index.redb                   (in-memory-or-disk chunk index)
...
- Packs in storage not in pack-list.

Findings are reported with suggested remediations. `crab fsck --repair` attempts safe repairs (e.g., re-adding missing entries to manifests).
...
**Option B: Cross-region replication at the bucket level.** AWS S3 CRR, GCS Turbo Replication. Writes to the primary region replicate to secondaries. Reads can go to local replica. Cons: replication is eventual (seconds to minutes), so reads-after-writes from other regions may miss. Ref updates are the risky case.

**Option C: Per-region repo mirrors with a promotion mechanism.** Outside v1 scope.

crab v1 officially supports Option A. Users choosing Option B are on their own to accept the consistency tradeoff; crab doesn’t know about replication and will behave strangely during the replication window for cross-region reads.
...
1. **On-disk chunk index.** At what repo size does the in-memory index become prohibitive? Probably ~5 TB based on 40 bytes/chunk. Benchmark `redb` as the fallback.
...
1. **Partial clones.** The proof-gated client-side protocol-v2 profile now
   supports `--filter=blob:none`; it omits ordinary blobs and serves later
   raw-OID requests from immutable visibility proof. The `crab clone` wrapper's
   lazy pointer checkout remains a separate policy; interaction with the
   clean/smudge filter is documented in the Git integration guide.
