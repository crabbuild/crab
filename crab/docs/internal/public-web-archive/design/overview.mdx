...
**Source:** `crab/src/git/clean.rs`, `crab/src/engine/chunker.rs`,
...
- Push locks (short-TTL leases in S3) serialize concurrent pushes to the
  same ref. The lock file lives at `locks/refs/{ref}/lock` and contains a
  JSON payload with holder identity and expiry timestamp.
...
**Source:** `crab/src/git/push.rs`, `crab/src/coordination/push_lock.rs`,
`crab/src/coordination/cas.rs`, `crab/src/git/push_manifest.rs`
...
**Source (existing infrastructure):** `crab/src/vfs/mod.rs` (stub),
...
pattern implemented in `coordination/cas.rs`:
...
of CAS. A push lock is a JSON file at `locks/refs/{ref}/lock` containing:
...
- `fsck` cleans up orphaned locks

This prevents two pushers from racing through the entire 14-step pipeline
only to have one fail at the final ref CAS. The lock serializes pushes to
the same ref early, saving wasted upload bandwidth.
...
| ChunkIndex (redb)  | chunk-hash → xorb-ref mapping  | Shard install         | Clean (dedup)             |
...
The `PersistentChunkIndex` is a redb database at
`~/.cache/crab/{bucket}/{repo}/chunk-index.redb`. It maps chunk hashes to
...
Tables:
- `chunks_v1`: chunk hash (32 bytes) → xorb hash (32 bytes) + chunk index (4 bytes LE)
- `shards_v1`: shard hash (32 bytes) → empty value (presence marker)
- `meta_v1`: string key → string value (schema version)

The index is populated incrementally as shards are installed (during fetch
and after push). Schema mismatches trigger a full rebuild from the shard
cache.
...
| Process killed (SIGKILL)         | Push lock may be orphaned                  | Lock expires after TTL; fsck cleans up      |
...
| `crab repack`        | Repack objects in remote store                   | Stub            |
...
| CDC chunker (gearhash)             | `engine/chunker.rs`                 | 64 KiB target, SIMD-accelerated          |
...
| CAS update loop                    | `coordination/cas.rs`               | Jittered backoff, bounded retries        |
...
| Persistent chunk index             | `metadata/persistent_chunk_index.rs`| redb-backed, schema migration            |
| Xorb format types                  | `storage/xorb/format.rs`            | MerkleHash, XorbRef, ChunkMeta           |
...
5. **FUSE mount** — `crab mount` provides a read-only filesystem where
   ...
   `crab/src/vfs/` module, reuse `FileIndexResolver` and `XorbFetcher`
   traits from smudge pipeline.

**Extension points:**
- `crab/src/vfs/` — FUSE mount logic (currently a stub)
- `crab/src/git/smudge.rs` — `SmudgeQueue`, `FileIndexResolver`,
...
- Config system — per-remote storage backend configuration
- Push pipeline — write to multiple backends (primary + replica)
- Fetch pipeline — read from nearest backend
...
│   └── xorb/
│       └── format.rs    MerkleHash, XorbRef, ChunkMeta, XorbInfo
├── metadata/            persistent indexes
│   ├── refs.rs          S3-backed ref store (CAS)
│   ├── persistent_chunk_index.rs  redb chunk→xorb index
│   └── chunk_index.rs   in-memory chunk index
├── coordination/        distributed coordination
│   ├── cas.rs           CAS update loop (jittered backoff)
