...
- [crab — Push Pipeline Deep Dive](#crab--push-pipeline-deep-dive)
  ...
  - [4. The 14-Step Push Pipeline](#4-the-14-step-push-pipeline)
    ...
    - [Step-by-Step Detail](#step-by-step-detail)
      ...
      - [Step 3: Pre-Push Shard Sync](#step-3-pre-push-shard-sync)
      - [Step 4: Classify Chunks](#step-4-classify-chunks)
      ...
      - [Step 9: Upload Shards and File-Index](#step-9-upload-shards-and-file-index)
  ...
  - [6. Phase Diagrams](#6-phase-diagrams)
    - [Phase 1: Classify (Steps 1–4)](#phase-1-classify-steps-14)
  ...
  - [11. Hardening Roadmap](#11-hardening-roadmap)
    - [Phase 1: Correctness and Resilience (Ship-Blocking)](#phase-1-correctness-and-resilience-ship-blocking)
    - [Phase 2: Performance (First 10x)](#phase-2-performance-first-10x)
    - [Phase 3: Performance (Next 10x)](#phase-3-performance-next-10x)
    - [Phase 4: Scale (100K+ Files)](#phase-4-scale-100k-files)
  ...
  - [14. Concurrent Push Scenarios](#14-concurrent-push-scenarios)
    ...
    - [Scenario 2: Two Pushers to the Same Ref (Concurrent, Current Behavior)](#scenario-2-two-pushers-to-the-same-ref-concurrent-current-behavior)
  ...
  - [15. Worked Example: Pushing a 2 GB Model Update](#15-worked-example-pushing-a-2-gb-model-update)
    ...
    - [Step 9: Upload Shard + File-Index](#step-9-upload-shard--file-index)
...
1. **Uploads chunk data** — xorbs (compressed chunk aggregates), shards
   (reconstruction metadata), and file-index entries are written to object
   storage. This is the "data plane."

2. **Uploads Git objects** — a standard Git packfile containing commits,
   trees, and pointer blobs is written to object storage. This is the
   "control plane."
...
The pipeline is implemented as a 14-step orchestrator in `PushPipeline`
(`crab/src/git/push.rs`), generic over a `ShardSyncer` trait for the
pre-push shard sync.
...
**Source:** `crab/src/git/clean.rs`, `crab/src/engine/staging/mod.rs`,
`crab/src/engine/staging/index.rs`

-----

## 4. The 14-Step Push Pipeline

The push pipeline is the core of `git push`. It is implemented as
`PushPipeline<S: ShardSyncer>` in `push.rs` and executes 14 steps
...
**Source:** `crab/src/git/walk.rs` (`walk_reachable`),
...
#### Step 3: Pre-Push Shard Sync

**Purpose:** Refresh the local `ChunkIndex` from remote shards so the
dedup classifier in step 4 has up-to-date knowledge of what's already
on the remote.

**How it works:**

```
ShardSyncer::sync_shards()           (optional generic syncer for tests)
    │
    ▼
GET shard-list manifest (ETag-cached per repo under repos/{repo_hash}/)
    │
    ▼
PersistentChunkIndex (redb)          (open or create per-repo)
    │
    ├── pre-populate in-memory ChunkIndex from persistent entries
    │
    ▼
ShardSynchronizer::sync
    │
    ├── generation-based incremental sync (only new shards)
    ├── download new shards into content-addressed cache
    ├── parse shards and install entries into ChunkIndex
    ├── if in-memory ceiling exceeded → spill to on-disk MDBShardFile
    │   handles (the "tier 3" shard_files vec)
    └── update persistent index with new installed shards
```

**Three-tier dedup state after step 3:**

| Tier | Storage                      | Lookup cost  | Lifetime           |
|------|------------------------------|--------------|--------------------|
| 1    | In-memory `ChunkIndex`       | HashMap O(1) | Per push           |
| 2    | `PersistentChunkIndex` (redb)| Disk B-tree  | Across pushes      |
| 3    | On-disk `MDBShardFile` handles | Interpolation search | Per push |

**Failure handling:** If the shard-list read fails, the push continues
with a partial ChunkIndex. Some chunks that already exist on the remote
may be re-uploaded — wasteful but correct.

**Source:** `crab/src/git/push.rs` (`shard_sync`, `open_persistent_chunk_index`,
`populate_from_persistent_index`, `run_pre_push_shard_sync`)
...
│    → detected via three-tier dedup lookup:              │
│      tier 1: in-memory ChunkIndex                       │
│      tier 2: PersistentChunkIndex (redb)                │
│      tier 3: on-disk MDBShardFile handles               │
│    → or via optional cache-service query_known_chunks   │
│                                                         │
...
dedup_ctx = DedupContext {
    chunk_index,          // tier 1
    persistent_index,     // tier 2
    shard_files,          // tier 3
}
classifier = Classifier::new()

// Optional: query cache-service for pre-seeding class A
if caching_store: cache_known = cs.query_known_chunks(all_hashes).await

for ptr in pointers:
    for chunk_hash in staging.chunks_for_file(ptr.file_hash):
        if cache_known.contains(chunk_hash):
            → Class A (cache-service dedup)
        else match classifier.classify_with_context(chunk_hash, &dedup_ctx):
...
// The class-C set is stored for step 5 to filter against.
// But only if at least one dedup tier is populated — otherwise
// step 5 falls back to packing everything.
if chunk_index or persistent_index or shard_files populated:
...
`crab/src/engine/dedup.rs` (`Classifier`, `DedupContext`,
`ChunkClass`, `lookup_three_tier`)
...
xorbs.retain(|x| !existing.contains(&x.hash))
metrics.xorbs_skipped += existing.len()
...
#### Step 9: Upload Shards and File-Index

**Purpose:** Upload shard files and per-file index entries to S3.

**How it works:**

```
For each shard (from step 8)  ──┐
For each pointer → file-index  ──┤  parallel, bounded by upload_concurrency
                                  │  via futures_util::buffer_unordered
                                  ▼
                             {prefix}/shards/{shard_hash}.shard
                             {prefix}/file-index/{file_hash}  (32 bytes)
```

Both shards and file-index entries run through the same
`buffer_unordered(upload_concurrency)` stream, so N uploads fly in
parallel regardless of which type they are.

**Local + optional remote cache warming.** Immediately after uploading,
the same bytes are written to:

1. The local content-addressed cache
   (`~/.cache/crab/...`) so the next hydrate/diff reads locally.
2. The cache service, if one is configured
   (`CachingStore::put`), so peers reading via the cache hit warm data.
...
**Purpose:** Generate and upload a standard Git packfile containing
...
1. Compute remote object set:
   GET {prefix}/manifests/pack-list → PackList
...
2. Generate incremental pack via git:
   git rev-list {tips} --objects --not {remote_tips}
     | git pack-objects --stdout
   → pack_bytes (only new commits, trees, and pointer blobs)
...
4. Install pack locally:
   Copy pack + idx into .git/objects/pack/ so the NEXT push can
   compute an incremental remote-object set without a round-trip.
   Soft-fails: any error logged and swallowed (worst case: next push
   uploads a full pack instead of a delta).
```

**Thin packs (optional).** When `PushConfig::thin_packs` is enabled,
pack-objects is passed `--thin` and the remote object set becomes the
delta-base set. Thin packs can be dramatically smaller but require the
remote to re-index on fetch.

**Source:** `crab/src/git/push.rs` (`upload_packs`, `compute_remote_objects`),
`crab/src/git/pack_gen.rs` (`generate_push_pack`,
`compute_remote_object_set`, `install_pack_locally`)
...
6. Append the new pack entry (from step 10) to the bulk pack list.
...
5. On CAS conflict (HTTP 412 — concurrent push):
   ...
   - Max retries: `config.max_cas_retries` (default 10).
...
- Stop heartbeat task
- Release push lock (DELETE lock file)
- Move staging data to cache (stub)
- Install shards into local ChunkIndex (stub)
...
```
 ...
 │                               ┌──────────┐   ┌──────────────────┐ │
 │                               │ Step 8   │──►│ Step 9           │ │
 │                               │ Build    │   │ Upload shards    │─┼──► shards/{hash}.shard
 │                               │ shard    │   │ + file-index     │─┼──► file-index/{hash}
 │                               └──────────┘   └──────────────────┘ │
 │                                                                   │
 │  ┌──────────────────┐                                             │
 │  │ Step 10          │                                             │
...
```
  ...
  (index.db)        │                                      │
                    │                                      │
  remote ─────────► │  3. shard sync: refresh ChunkIndex   │
  (shard-list)      │     (best-effort, warn on failure)   │
                    │                                      │
  ChunkIndex ─────► │  4. classify: A/B/C per chunk        │
                    │     A = remote hit (skip)            │
...
```
                    ...
                    │                                      │
                    │  9. Upload shards + file-index:      │
                    │     PUT shards/{hash}.shard          │──► shards/
                    │     PUT file-index/{file_hash}       │──► file-index/
...
The push lock prevents concurrent pushers from racing on the same ref.
It is a short-TTL lease stored as a JSON object in S3:

```
Path: {prefix}/locks/refs/{ref_name}/lock
...
PUT lock (holder=A, expires=T+5m)
...
2. DELETE lock
3. PUT lock → { holder: "B", expires_at: T+5m }
```
...
│
├── file-index/
│   └── {file_hash}                 ← 32 raw bytes (shard hash)
│
├── manifests/
...
| `file-index/*`      | Immutable  | PUT once, never updated |
...
| Step 10 (pack upload)  | Xorbs + shards + pack uploaded      | Orphaned objects (GC cleans up)   |
...
3. **Lock always released.** The `on_failure` path always stops the
   heartbeat and releases the lock. If the process crashes, the lock
   expires via TTL.
...
`crab/src/coordination/push_lock.rs`.
...
| `push_thin_packs` | false | Enable thin pack generation (delta against remote bases) |
...
| Holder ID | `push-{uuid}` | Unique identifier per push attempt |
...
| Staging area | `engine/staging/mod.rs` | `StagingArea`, `stage_chunks_batch`, `get_chunk` |
| Staging index | `engine/staging/index.rs` | `Index`, `chunks_for_file`, `locate` |
...
| CAS loop | `coordination/cas.rs` | `cas_update`, `cas_update_default` |
...
### Step 9: Upload Shard + File-Index

```
PUT shards/0xS1.shard (180 KB)           ~50ms
PUT file-index/0xA1B2... (32 bytes)      ~50ms
...
```
git rev-list def456 --objects | git pack-objects --stdout
...
| 3 | Staging never mutated until push succeeds | Steps 1–12 only read from staging; step 13 moves to cache |
...
→ Any chunk in `known` is treated as class A, bypassing even the
  three-tier local dedup.
```
...
**Source:** `crab/src/git/push_state.rs`,
...
| Persistent chunk index | `metadata/persistent_chunk_index.rs` | `PersistentChunkIndex::open_or_create`, `load_all`, `install_shards` |
