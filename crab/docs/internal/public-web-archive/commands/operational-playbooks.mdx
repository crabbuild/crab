...
crab restripe --profile=ml --dry-run

# 3. Execute the restripe
crab restripe --profile=ml --apply

# 4. If interrupted (SIGTERM/crash), resume
crab restripe --profile=ml --resume

# 5. Verify post-restripe hydration
crab hydrate --all
crab fsck
...
crab restripe --drop-journal --yes-really
...
| Restripe xorbs | `crab restripe --apply` | Medium |
...
| `restripe --apply` | `push` | Yes — reconciliation handles it |
| `restripe --apply` | `gc` | No — `ConcurrentMaintenance` error |
| `restripe --apply` | `restripe --apply` | No — `RestripeAlreadyInProgress` error |
| `gc` | `tier plan --apply` | Yes — independent operations |

**If you see `ConcurrentMaintenance`:**

```bash
# Check if a restripe is running
ls .crab/restripe/journal.db

# If the process crashed, the lock may be stale
# Resume or abort the restripe first, then run GC
crab restripe --resume   # or --abort
crab gc
```
