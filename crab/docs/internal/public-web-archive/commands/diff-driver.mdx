...
```
crab diff-driver <path> <old-file> <new-file> <old-hex> <old-mode> <new-hex> <new-mode>
```
...
The diff driver is automatically registered when you run `crab track`. The
`.gitattributes` entry includes `diff=crab`:
...
3. If both are pointers, it resolves reconstruction terms from shard metadata.
4. Compares chunk lists to determine added, removed, and modified segments.
5. Formats the output to stdout.
6. If neither file is a pointer, falls back to showing a file size difference.
