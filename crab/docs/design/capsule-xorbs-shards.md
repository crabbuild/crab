...
| Shallow, deepen, unshallow, filtered/partial, and raw-object/promisor fetch | Initial filtered transfer uses terminal Git protocol-v2. Later raw-OID recovery uses Git's line-oriented helper re-entry, pins one authenticated capsule view, authorizes the OID against visible-ref closure, generates only its selected pack, and installs the pack plus `.promisor` sidecar atomically. RustFS qualification covers the initial filter matrix and lazy retrieval | Complete released-shape, older-Git, hosted-provider, interrupted-resume, hidden-ref, cancellation, and adversarial transport qualification | Git compatibility matrix for every fetch mode, including lazy recovery after process restart, interrupted installation, hidden-only objects, and adversarial missing objects |
