...
| Managed/protected push and active-active publication | Direct active-active pushes bind the exact v2 base root, transaction, activation, and capsule run in coordinator truth, materialize per-ref heads after consensus, and retain ordered regional repair records. Active-active mirror plans replicate their immutable intent and repair terminal receipts after a replacement regional activation. Protected admission reads v2 per-ref state and persists the exact root digest plus authorized old OIDs. The client stages the thin capsule and its Xet/LFS dependencies under the authorization grant without mutating GC or ref state. Direct-source verification binds the staged run, Git closure and visibility, changed paths, Crab shard/xorb closure, LFS bodies, and complete staged-object inventory. Direct protected finalize revalidates its evidence, promotes immutable dependencies, registers verified shard roots, publishes through the canonical per-ref transaction, and recognizes an exact already-visible transaction on retry | Commit protected active-active capsules through coordinator truth, synthesize filtered-view source capsules, and complete Crab Auth and managed-provider qualification | Deny/allow/stale-policy races, lost responses, regional failover, ordered repair, receipt recovery, and all-old/all-new multi-ref visibility |
