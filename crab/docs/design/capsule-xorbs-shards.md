...
| Full clone, fetch, pull, and ref advertisement | Checkpoint snapshots retain exact incremental transition history, but the current reader authenticates that metadata by downloading and hashing the complete checkpoint object, including its Git pack payload | Hard-cut the checkpoint pointer/codec to bind one contiguous control suffix, range-load that suffix for planning, verify selected pack ranges against footer-bound block hashes, then repeat the 5,000-push qualification and bound many-ref head capture | Repositories with thousands of refs; exact refs, byte-identical checkout, strict fsck, bounded requests and memory; metadata-only open transfers zero checkpoint pack bytes; post-checkpoint incremental fetch request and byte counts remain proportional to its selected delta |
...
batched-run implementation meets the 500-push foreground budget, but the
checkpoint control-suffix change and a fresh 5,000-push replay remain required.
