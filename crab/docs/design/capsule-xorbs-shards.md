...
| S3 gateway read and mutation | Repository reads select a present v2 root exclusively, open authenticated capsule packs and refs through the shared remote Git reader, cache by the exact v2 state digest, and fail closed instead of falling back to v1. Mutation publication still depends on the v1 manifest/journal | Publish gateway mutations and multipart recovery through v2 ref transactions, then remove the v1 write owner | S3 read/list/write/delete/multipart semantics, corrupt-v2 rejection, concurrent mutations, restart, and clone/fsck verification |
