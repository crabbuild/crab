...
| Repack, repository GC, bucket GC, and fsck | V2 checkpoint publication writes one authenticated history segment in parallel with the checkpoint; repository GC walks the bounded segment chain and retains every referenced checkpoint and capsule run; fsck authenticates the chain and all immutable dependencies before reporting the repository clean. Current pointer catalogs are append-only for shard/xorb identities, so bucket GC retains historical external-data dependencies through the current authenticated catalog | Complete crash/fault, multi-segment, pruning, and forced-GC concurrency qualification | Injection at each publication boundary; resurrection, restart, no reachable deletion, and bounded writer pause |
...
| Doctor, history inspection/restore, and v1-to-v2 cutover | Remote doctor selects and verifies a present v2 root before considering the validated v1 layout, identifies the v2 generation, accepts v2-only repositories, and fails closed on corrupt v2 authority. Checkpoint maintenance now publishes a deterministic authenticated history-segment chain without adding a foreground push request; repository GC retains its checkpoint/capsule closure and fsck verifies that closure | Implement transaction/ref and checkpoint selectors, restore-as-new-publication, retention/pruning policy, doctor reporting, and the offline verified one-way migration command | Migrate a populated v1 repository, reject dual authority, recover retained per-ref and checkpoint history, prune without reachable deletion, then clone/hydrate/fsck |
