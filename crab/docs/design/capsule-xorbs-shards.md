...
| Managed/protected push and active-active publication | Direct active-active pushes bind the exact v2 base root, transaction, activation, and capsule run in coordinator truth, materialize per-ref heads after consensus, and retain ordered regional repair records. Active-active mirror plans replicate their immutable intent and repair terminal receipts after a replacement regional activation. Protected admission now reads v2 per-ref state and persists the exact root digest plus authorized old OIDs; capsule verification/finalize remains rejected | Move protected verify/finalize onto the same prepared-capsule adapter and complete managed-provider qualification | Deny/allow/stale-policy races, lost responses, regional failover, ordered repair, receipt recovery, and all-old/all-new multi-ref visibility |
