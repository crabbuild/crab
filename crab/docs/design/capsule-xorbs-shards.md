...
| FUSE/NFS mount | Shared v2 file-index and hydrator wiring implemented; remote mount contexts pin an authenticated control-view catalog and all external shard/xorb reads honor archive-restore admission | Qualify range reads, cold/warm cache, eviction, cancellation, unmount, replica failover, and restored-tier objects | Mount/read/stat/range/concurrent-reader suite on every supported mount platform and provider |
...
| Tiering and archive restore | Canonical xorb identity is reusable; hydrate now gates both shard and xorb reads through the restore orchestrator, but v2 reachability integration is unqualified | Drive lifecycle and restore decisions from v2 reachability while keeping restore state non-authoritative | Transition/restore/hydrate/mount/GC race tests for every supported storage class |
