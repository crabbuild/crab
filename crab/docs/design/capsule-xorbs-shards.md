...
| Managed/protected push and active-active publication | Direct active-active pushes now bind the exact v2 base root, transaction, activation, and capsule run in coordinator truth, materialize per-ref heads after consensus, and retain ordered regional repair records; protected publication and active-active mirror-plan receipts remain rejected | Move protected authorization/finalize onto the same prepared-capsule adapter, bind mirror-plan terminal receipts to coordinator decisions, and complete managed-provider qualification | Deny/allow/stale-policy races, lost responses, regional failover, ordered repair, receipt recovery, and all-old/all-new multi-ref visibility |
