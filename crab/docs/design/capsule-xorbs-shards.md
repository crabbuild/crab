...
| Replica selection, readiness, repair, and active-active reconciliation | Read selection now requires an exact authenticated v2 root/ref/checkpoint/capsule state digest, rechecks that digest on cache hits, and verifies every cataloged shard/xorb body before caching immutable-dependency readiness; repair and active-active materialization still use v1 manifest projections | Repair immutable v2 dependencies before authority, replace regional manifest projection, and qualify failover/failback | Lag, partial replication, corrupt replica, failover/failback, repair, and concurrent publication matrix |
