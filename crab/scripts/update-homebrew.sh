...
#
# Usage:
#   ./scripts/update-homebrew.sh              # uses latest release tag
#   ./scripts/update-homebrew.sh v1.0.1       # explicit tag
#   ./scripts/update-homebrew.sh --local      # use local dist/ checksums
#   ./scripts/update-homebrew.sh --dry-run    # render formula, don't push
...
if [[ "$USE_LOCAL" == true ]]; then
    if [[ ! -d "$DIST_DIR" ]]; then
        error "dist/ not found. Run ./scripts/release.sh --dry-run first."
    fi
