...
def check_release_workflow(root: Path) -> list[str]:
    ...
    for needle in (
        ...
        "crab/scripts/release/generate-release-manifest.py",
        'gh release upload "$TAG" dist/crab-release.json',
        'gh release edit "$TAG"',
        "--draft=false",
        ...
        "AWS_ACCESS_KEY_ID: ${{ env.WORKFLOW_RUSTFS_ACCESS_KEY }}",
        "AWS_SECRET_ACCESS_KEY: ${{ env.WORKFLOW_RUSTFS_SECRET_KEY }}",
        "AWS_DEFAULT_REGION: ${{ env.WORKFLOW_RUSTFS_REGION }}",
...
def checks(root: Path) -> list[TextCheck]:
    return [
        ...
        TextCheck(
            "self-upgrader uses the public release manifest without the GitHub API",
            root / "crab" / "src" / "cmd" / "upgrade.rs",
            contains=(
                "https://github.com/crabbuild/crab/releases/latest/download/crab-release.json",
                "https://github.com/crabbuild/crab/releases/download",
            ),
            excludes=("api.github.com", "crabbuild/crab-release"),
        ),
        TextCheck(
