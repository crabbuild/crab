...
_crab_completions() {
    ...
    subcmds="init add reset clone mirror doctor du track untrack stat gc fsck compact repack optimize tier metadb cache staging errors version config status hydrate diff dehydrate env ls-files fetch prune logs install uninstall lock unlock locks migrate push ship import run exp workflow params metrics lfs mount unmount daemon login logout auth filter-process"
...
set -l crab_subcmds init add reset clone mirror doctor du track untrack status hydrate dehydrate ship push optimize install uninstall
