...
const RELEASE_API_URL: &str = "https://api.github.com/repos/crabbuild/crab/releases/latest";
...
pub async fn run_update(args: UpdateArgs) -> Result<()> {
    ...
    let Some(release) = fetch_latest_release(&client).await? else {
        let payload = UpdatePayload {
            ...
            asset: None,
            message: "No crab release is available from crabbuild/crab.".to_owned(),
        };
...
mod tests {
    ...
    fn release_with_assets(names: &[&str]) -> ReleaseResponse {
        ReleaseResponse {
            ...
            html_url: Some("https://github.com/crabbuild/crab/releases/tag/v1.2.3".to_owned()),
            assets: names
                .iter()
