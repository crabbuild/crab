...
meta:
  contentType: How-to
  goal: "Use GCP Credentials in an existing Crab repository and verify the result."
...
Treat this as a local credential-state check. To verify the complete path,
perform a read-only operation against the configured GCS-backed repository and
confirm that it succeeds under the expected service account. A successful
token exchange does not by itself prove bucket IAM access.
