...
meta:
  contentType: How-to
  goal: "Use Logging In in an existing Crab repository and verify the result."
...
After login, verify both local state and one read-only managed-service request:
...
The first command describes cached authentication state. The second crosses
service discovery, token refresh, API compatibility, and authorization. For a
self-hosted service, replace `crab.build` with its installed authority. Keep the
authority explicit during diagnosis so an active profile change cannot make the
probe address a different service.

Login does not create repository membership or object-store permissions. If
authentication succeeds but the read-only request is forbidden, inspect the
principal's organization role instead of repeating browser login.
