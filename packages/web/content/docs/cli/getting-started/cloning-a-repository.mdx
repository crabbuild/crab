...
description: "Clone a Crab repository with a lazy checkout, then hydrate selected file content on demand."
meta:
  contentType: Tutorial
  goal: "Complete Cloning a Repository and verify each important state transition."
  audience: "Crab users following this workflow for the first time."
---

# Cloning a Repository

Cloning a Crab repository downloads Git metadata and lightweight pointer blobs instead of hydrating every managed file. Clone time therefore depends on repository history and metadata rather than the total bytes stored in Crab-managed files. The file content remains in cloud storage until you explicitly hydrate it.
...
```bash
crab hydrate '*.safetensors'    # hydrate model files only
