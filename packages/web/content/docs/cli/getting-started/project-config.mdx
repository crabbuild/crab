...
meta:
  ...
  goal: "Understand Project Configuration and the system boundaries it defines."
...
AWS profile names are machine-specific and do not belong in committed project
configuration. Select one with `crab configure --aws-profile ml-team`, `crab
config set auth.aws_profile ml-team`, or `AWS_PROFILE=ml-team`.
...
Collaborators clone Git metadata and pointers, then hydrate the specific model checkpoint they need.
