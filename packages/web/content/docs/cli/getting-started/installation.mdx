...
meta:
  ...
  goal: "Complete Installation & Setup and verify each important state transition."
...
| Remote URL | `crab.toml` | Points to your cloud bucket |
| Config | `.crab/local.toml` | Local Crab settings |
| Staging area | `.crab/staging/` | Unpublished chunks awaiting push; not a disposable cache |
...
# The filter driver is already registered globally
```
...
This configures the smudge filter to pass pointers through unchanged. Git can check out the small pointer blobs, and files remain as pointers until you explicitly hydrate them:
