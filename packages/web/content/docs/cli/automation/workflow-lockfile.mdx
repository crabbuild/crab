...
meta:
  ...
  goal: "Find the supported syntax and behavior for Workflow Lockfile."
...
| `--update-config` | | `false` | Also set `[workflow] lockfile = "split"` in `.crab/local.toml` |
| `--json` | | `false` | Emit structured JSON output |
...
`.crab/local.toml` so subsequent `crab run` invocations use the new layout.
