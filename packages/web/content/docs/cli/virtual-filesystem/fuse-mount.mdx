...
description: "Mount a Crab repository as a virtual filesystem and read file content on demand without explicit hydration."
meta:
  contentType: How-to
  goal: "Use Virtual Filesystem Mount in an existing Crab repository and verify the result."
  audience: "Crab users completing this task in an existing repository."
---

# Virtual Filesystem Mount

The virtual filesystem mount exposes every path in a Crab repository without downloading all file content upfront. Files appear at their full size in directory listings, but content is fetched from cloud storage when a reader opens the relevant ranges. Startup and read latency still depend on repository metadata, the selected backend, network conditions, and cache state.
...
1. **Directory structure comes from metadata**: Crab builds a snapshot from Git tree objects. `ls` and `find` do not require full file hydration.
...
|-----------|---------|-------|
| `ls` / directory listing | Metadata-only | Served from the in-memory snapshot |
...
| Browsing or exploring a large repository | Mount for on-demand reads without an upfront full download |
