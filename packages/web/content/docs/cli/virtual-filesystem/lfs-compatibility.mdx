...
- Transfer-agent setup is Git config, so CI and collaborators may need
  `crab lfs install`.
- `crab lfs install` and `crab lfs update` fail if an unmanaged `pre-push`
  hook already exists. Merge `crab lfs pre-push "$@" || exit $?` into the hook
  manually, or use `--force` only when replacing it is intentional. Crab's own
  mirror hook is composed automatically.
