...
- `crab doctor --cost` uses the embedded table and a live remote inventory to
  estimate monthly storage costs and recommendation deltas. The live inventory
  includes shared Crab data plus every object under the configured repository
  prefix, including capsule-v2 metadata, without charging sibling repository
  prefixes to the report.
