...
meta:
  ...
  goal: "Find the supported syntax and behavior for crab push."
...
| `--follow-tags` | Disabled | Also push missing annotated tags whose commits are reachable from the refs being pushed |
| `-v, --verbose` | Disabled | Show per-step timing and per-file progress |
| `--no-incremental` | Disabled | Publish the complete outgoing Git and LFS closure instead of excluding objects reachable from current remote tips |
