...
2. Submits estimated bytes and objects for service-owned quota and concurrency admission.
3. Prepares a durable push session and exact staging scope.
4. Flushes and verifies every required staged xorb and bundle object.
5. Asks the service to recompute policy-relevant paths and validate Git connectivity.
6. Finalizes through service-owned canonical credentials and per-ref serialization.
...
weighted by configured worker width and the estimated xorb memory window, to
bound concurrent upload pressure. These slots provide repository workload
fairness within one cloud-credential trust domain; they do not identify or
enforce quotas for separate teams. Use a managed authority when authenticated
organization-level admission is required. Crab does not switch between managed
and direct behavior after a failure.
