...
meta:
  ...
  goal: "Find the supported syntax and behavior for crab adopt."
...
| `--rewrite-history` | Rewrite all selected refs with Crab pointers; requires `--force` and a clean tree |
...
`--rewrite-history` delegates to the same verified fast-export/fast-import
engine as `crab migrate import`. It stages every selected file version in
`.crab/staging`, rewrites all refs, and requires collaborators to re-clone
after a force push.
