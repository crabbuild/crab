...
The command reads the canonical repository URL from `crab.toml`. For a
...
Post-fetch chunk-index warming reads the v2 root and authenticated pointer
catalog when present. It reads the v1 manifest and journal only when no v2 root
exists; corrupt v2 authority is an error rather than a fallback signal.
