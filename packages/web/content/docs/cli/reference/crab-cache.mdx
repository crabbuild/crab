...
Cleanup checks all configured cache roots before removing data. An active
mirror using an overlapping cache causes a nonzero, retryable refusal without
deletion from any configured root. New mirror operations also refuse to start
while cleanup owns the cache. Wait for the active operation and retry; do not
delete its lock files.

Cleanup retains `.crab-cache-use.lock` and `.crab-cache-clean.lock` markers
and the directories needed to contain them. Reported file and byte counts
exclude these markers. An idle mirror cache can be removed or emptied; the
next mirror invocation rebuilds it, including when only nested coordination
markers remain. Cloud objects and repository refs are not
deleted. This coordination covers mirror directory owners, not arbitrary
external programs or every concurrent cache fill. Cancellation or an I/O
error during deletion stops further work but does not restore removed cache
data.
...
verified as they are scanned. I/O failures fail the command.
