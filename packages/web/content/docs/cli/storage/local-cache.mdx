...
Shows the xet range-cache limit and usage plus Crab object-cache counts and
bytes. The current command opens the xet cache before scanning it and does not
include every SQLite/index/hint file in its total. It does not repair or delete
entries.
