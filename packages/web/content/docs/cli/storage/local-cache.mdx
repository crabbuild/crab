...
Shows the shared budget, decoded-range usage, and object-cache counts and
payload bytes, including chunk payloads. Inspection leaves missing roots
missing and does not open cache databases, initialize the writable range
cache, or repair entries. A failed range or object scan is reported without
hiding the other group's results, and causes an unsuccessful exit. These are
not full disk-usage totals: manifest bytes, databases, hints, bloom files,
temporaries, and retained state are excluded. See
