...
7. Rebinds the existing Git visibility and complete commit-graph proofs to the
   replacement generation, then publishes exact locators and a generation
   receipt. Repairable post-CAS failures never invalidate the committed
   manifest.
