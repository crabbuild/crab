...
2. Selects the smallest suffix whose replacement restores a factor-two
   geometric progression.
3. Downloads and verifies only that suffix with bounded concurrency after a
   free-space preflight; stable large packs stay in object storage and are not
   rewritten.
4. Generates one pack from the exact selected OID union and verifies that no
   object was added or lost.
5. Uploads only newly generated pack, index, and reverse-index objects, and
   repairs missing metadata sidecars.
6. Stages a zero-edit catalog visibility handoff, then atomically updates the
   manifest using compare-and-swap (CAS). The handoff preserves incremental ref
   history across the new pack-index identity without serializing the complete
   OID dictionary.
7. Completes the visibility handoff, rebinds the commit-graph proof, then
   publishes exact locators and a generation receipt. Repack rebinds a legacy
   materialized visibility proof when no catalog-bound proof exists. Repairable
   post-CAS failures never invalidate the committed manifest.
