...
meta:
  contentType: How-to
  goal: "Use Garbage Collection in an existing Crab repository and verify the result."
...
For Git packs, repo-scoped dry-run output separates physical bytes into
`active`, `retained_history`, `grace_period`, and `collectible` classes.
Manifest history is the stale-pack inventory: it records immutable
generation/time evidence without introducing a second mutable liveness index.
GC still recomputes reachability from current, history, workflow, and
coordinator roots before treating any pack as collectible.
