...
Retention is unlimited by default, so prune does nothing until you set a cap:

```bash
crab config set cache.max_bytes 10737418240  # 10 GiB
```

Use `crab config set cache.max_bytes unlimited` to remove the cap. Explicit
`crab cache clean` can remove eligible payloads without a cap; it preserves
unknown files, live owners, and retained metadata rather than wiping the root.
