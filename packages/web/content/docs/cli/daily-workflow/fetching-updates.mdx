...
The local cache lives at `~/.cache/crab/` by default (configurable via `$CRAB_CACHE_DIR` or `.crab/local.toml`).
...
# Later hydration can reuse the local cache
crab hydrate 'models/**'
```
...
You can also set a maximum cache size in `.crab/local.toml`:
