...
meta:
  contentType: How-to
  goal: "Use Dehydrating Files in an existing Crab repository and verify the result."
...
- **Reduce branch-switch work**: Git can replace small pointers instead of materialized large files.
...
This reduces the amount of full file content Git must carry through a branch
change. A correctly configured Crab clean filter can keep unchanged hydrated
files clean in Git status, but pointer-only working trees also reduce the local
I/O required during checkout and merge.
