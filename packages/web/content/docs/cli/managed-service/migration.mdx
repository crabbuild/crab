...
meta:
  contentType: How-to
  goal: "Use Migrate to the Managed Service in an existing Crab repository and verify the result."
...
Two independent migrations are often confused:
...
edit `crab.toml`, or install a managed profile as a substitute for copying
and verifying data.
...
cat models-managed/crab.toml
```

Both identities must be `crab://crab.build/acme/models`. Re-cloning avoids a
mixed worktree whose Git remote, `crab.toml`, project configuration, and
...
The Python FastAPI endpoint under `crab/deploy/auth-service` serves existing enterprise
