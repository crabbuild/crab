...
meta:
  ...
  goal: "Preview the complete managed-service repository workflow."
  audience: "Teams evaluating Crab's planned hosted control plane."
---

# Managed Service Quickstart

<Callout type="preview" title="Managed Service is coming soon">
  We're putting the finishing touches on the managed service. In the meantime,
  explore this preview of the planned workflow. Some details may evolve before
  launch.
</Callout>
...
<DocsPathDiagram
  id="managed-repository-path"
  title="Managed repository path"
  caption="The service owns repository identity and canonical publication. The CLI transfers immutable objects with short-lived, scoped access."
  steps={[
    { label: "Sign in", detail: "OIDC session", tone: "control" },
    { label: "Provision", detail: "acme/models", tone: "store" },
    { label: "Transfer", detail: "immutable data", tone: "data" },
    { label: "Publish", detail: "canonical refs", tone: "safe" },
  ]}
/>
...
fetch` does not replace it; `crab fetch` reads `crab.toml` and downloads
