...
knowledgeCheck:
  question: "Which Git extension points does Crab use for a tracked model and a remote push?"
  options:
    - "A merge driver for both operations"
    - "A filter process for representation and a remote helper for repository transport"
    - "A custom Git fork that replaces the object database"
  answer: 1
  explanation: "Git invokes Crab through standard filter-process and remote-helper contracts; Crab does not patch Git."
meta:
...
Continue with [How do you work with Crab after the first push?](/library/daily-crab-workflow) to turn these integration points into a repeatable team workflow.
