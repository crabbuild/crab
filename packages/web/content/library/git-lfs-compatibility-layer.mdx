...
- **Bridge:** keep the LFS pointer and commit IDs; invoke Crab's local
  standalone transfer agent, which accesses the Crab object store directly.
...
`install` configures the local transfer agent, LFS filters, and a pre-push
