...
knowledgeCheck:
  question: "At what point does a prepared Crab push become visible to other readers?"
  options:
    - "When the first xorb upload starts"
    - "When the local commit is created"
    - "When the final ref transaction commits after dependency proof"
  answer: 2
  explanation: "Immutable data and metadata are prepared first. The ref transaction is the single visibility boundary."
meta:
...
Next, [see how local caches reduce the reads that prepare and verify these stages](/library/multi-layer-caching).
