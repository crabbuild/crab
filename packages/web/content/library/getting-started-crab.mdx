...
knowledgeCheck:
  question: "Where should the rule for tracking `*.safetensors` with Crab live?"
  options:
    - "In the committed `.gitattributes` file"
    - "Only in one developer's shell history"
    - "Inside the object-storage bucket policy"
  answer: 0
  explanation: "The committed attribute rule makes file representation shared and reviewable across contributors and CI."
meta:
...
You can instead use Crab's checksum-verifying installer on macOS or Linux:
...
On macOS or Linux, open a new shell if the installer changed your `PATH`, then verify both entry points:
...
```powershell
crab version
Get-Command git-remote-crab
```

The version command proves that the CLI runs. The helper lookup proves that Git can find the remote helper executable. Repository setup installs the filter configuration later.

Pin `CRAB_VERSION` in continuous integration and build images when you need repeatable automation. Read [Installation & Setup](/docs/cli/getting-started/installation) for custom install directories, global filter registration, mount helpers, and platform requirements.
...
For a more explicit verification workflow, continue with [How do you verify your first Crab push?](/library/first-crab-push). For command options, use the [`crab configure`](/docs/cli/reference/crab-configure), [`crab track`](/docs/cli/reference/crab-track), and [`crab hydrate`](/docs/cli/reference/crab-hydrate) references.
