...
knowledgeCheck:
  question: "What finally prevents a stale writer from replacing a newer branch tip?"
  options:
    - "Uploading its xorbs first"
    - "Waiting for the lease to expire"
    - "The expected-old comparison in the ref transaction"
  answer: 2
  explanation: "The lease reduces overlap. The expected-old check is the correctness boundary that rejects a plan based on stale history."
meta:
...
Next, [choose how much repository content should become local](/library/lazy-checkout-fuse).
